...
!.... ALL THE OPACITY ROUTINES ARE HERE

!.... 2019 SEP - UPDATED TO FIORELLA CASTELLI'S selectlines
!.... 2019 APR - REPLACED forall BY do concurrent
!.... 2017 JUL - CHANGED THE recl OF ALL FILES FROM BYTES TO 4-BYTE WORDS
!....          - UPDATE TO USE BOB'S gfallnlte.dat, AN ASCII FILE
...
!                KAPP BY REMOVING ALL THE LINE OPACITY CALLS
     ...
     &                                 sig_xl
      ...
      use physical_constants,    only: hyd_inu
...
!!!!  if(if_op(9) .and. freq .le. 3.28805d15) call coolop
     ...
     &                   a_xcont(1:ndepth)
      ...
      a_line(1:ndepth) = a_hline(1:ndepth) +
     &                   a_lines(1:ndepth) +
     &                   a_xline(1:ndepth)
...
!.... 2009 AUG - ADDED DOPPH, LIKE SYNTHE
!....            USE xnf INSTEAD OF xnfp
...
!-------------------------- hprof4 ARGUMENTS ---------------------------
      ...
      real(re_type), intent(in) :: hw_dop ! RENAMED, BUT LIKE SYNTHE
...
!-------------------------- hprof4 VARIABLES ---------------------------
      ...
      real(re_type), save :: t3nh2(max_d) ! LIKE SYNTHE
...
!-------------------------- hprof4 EXECUTION ---------------------------
     ...
     &                             sqrt(xne(1:ndepth) / t(1:ndepth)))
         ...
         do ki = 1, ndepth
            ...
            t43 = t4**0.3d0
            y1_s(ki) = t43 / xne16
!!!!        t3nhe(ki) = t43 * xnfp(ki, 3)
            t3nhe(ki) = t43 * xnf(ki, 3)     ! xnf LIKE SYNTHE
            t3nh2(ki) = t43 * xn_mol(ki, 86) ! 86 = H2 LIKE SYNTHE
...
!.... THESE HALF-WIDTHS ARE REALLY DNU/NU
      ...
      hw_res = resont * xnf(j, 1)              ! RESONANT
!.... ADJUSTMENT FROM SYNTHE
!.... APPROXIMATE THE GROUND STATE PARTITION FUNCTION AS 2
      hw_res = hw_res * 2.0d0                  ! MODIFIED RESONANT
      hw_stk = stark * fo(j)                   ! STARK
      hw_vdw = vdw * t3nhe(j)                  ! VAN DER WAALS
!.... ADJUSTMENT FROM SYNTHE
!.... GUESS THAT H2 IS TWICE AS STRONG AS HE
...
!.... FOLLOWING SYNTHE HPROF4 - PUTTING THESE BEFORE THE IF(FREQ .gt.
...
!!!!           hprof_res = hw_res * freq_nm / pi /             ! SYNTHE
!!!!                       (del**2 + hhw**2) * 1.77245d0 * dop ! SYNTHE
...
!.... TABULATED AT 200 CM-1 SPACING
     ...
     &                     xnf(j, 1)
!!!! &                     xnfp(j, 1) * 2.0d0
!.... 2.0D0 * XNFP(J, 1) CONVERTS PARTITION FUNCTION TO PARTICLE DENSITY
...
!.... THE FREQ TESTS ARE FROM SYNTHE
!.... IF(FREQ.GT.2.463E15)
...
!!!! &                     1.77245d0 * dop
     ...
     &                     (del**2 + hhw**2) * dop
         ...
         else if(.not. ifcore) then ! FOLLOWING SYNTHE VERSION
            hhw = freq_nm * hw_lor
            top = hhw
...
!.... TABULATED AT 100 CM-1 SPACING
     ...
     &                     spec_spacing * (freq - cutfreq)
               ...
               cutoff = 10.0d0**(cutoff - 14.0d0) / c_cm * xnf(j, 2)
...
!--------------------------- hfnm EXECUTION ----------------------------
      ...
      if(m .gt. n) then
         ...
         if(n .ne. n_last) then
            ...
            n_last = n
            m_last = -m ! TO FORCE THE NEXT SECTION TO BE DONE TOO
         end if
...
!******************* E N D  F U N C T I O N  H F N M *******************
      ...
      use atmosphere_parameters, only: ndepth
      use code_dimensions,       only: max_d
      use continuum_tables,      only: cont_tab, cont_wave
      use freq_set,              only: num_nu, waveset
      use state_vars,            only: xne
      use temp_vars,             only: hckt
      use txnxn_vars                 ! txnxn
      use var_types
      use xlines_vars                ! xlines
      use xnf_dop                    ! dopple, xnfdop
...
!--------------------------- INTERFACE BLOCK ---------------------------
      ...
      interface
         ...
         function voigt(v, a) result(voigt_out)
         use var_types
         real(re_type), intent(in)  :: a
         real(re_type), intent(in)  :: v
         real(re_type)              :: voigt_out
         end function voigt
...
!--------------------------- linop VARIABLES ---------------------------
      ...
      integer(in_type) :: ios13
      integer(in_type) :: iw
      integer(in_type) :: j
      integer(in_type) :: ki
      integer(in_type) :: line
      integer(in_type) :: nelion
      integer(in_type) :: nu
      integer(in_type) :: nucont

      logical :: if_j(max_d+1)

      real(re_type) :: adamp
      real(re_type) :: center
      real(re_type) :: congf
      real(re_type) :: cv
      real(re_type) :: dopwave
      real(re_type) :: elo
      real(re_type) :: gammar
      real(re_type) :: gammas
      real(re_type) :: gammaw
...
!--------------------------- linop EXECUTION ---------------------------
      ...
      write(6, '(/ a / a10, a11, a7, 5a10)') "linop",
     &   "line", "wave(nm)", "nelion", "congf", "elo", "gammar",
     &   "gammas", "gammaw"
      write(*, '(/ a / a10, a11, a7, 5a10)') "linop",
     &   "line", "wave(nm)", "nelion", "congf", "elo", "gammar",
     &   "gammas", "gammaw"

      if_j(1) = .false.
      xlines(1:ndepth, 1:num_nu) = 0.0d0

!.... txnxn MOVED FROM HERE TO main TO BE AVAILABLE FOR xlinop TOO
      ...
      file13_loop: do ! THE WHOLE FILE
         read(13, iostat=ios13) wlvac, nelion, congf, elo, gammar, 
     &                          gammas, gammaw
         if(ios13 .ne. 0) exit file13_loop
         line = line + 1

         if(mod(line, 100000) .eq. 1) then
            write(6, '(i10, f11.4, i7, 5es10.3)') line, wlvac,
     &            nelion, congf, elo, gammar, gammas, gammaw
            write(*, '(i10, f11.4, i7, 5es10.3)') line, wlvac,
     &            nelion, congf, elo, gammar, gammas, gammaw
         end if

         if(wlvac .lt. wl_last) then ! RESET FOR A NEW LINE LIST
            write(6, '(2(5x, a, f11.4))') "last line =", wl_last,
     &                                    " this line =", wlvac
            write(*, '(2(5x, a, f11.4))') "last line =", wl_last, 
     &                                    " this line =", wlvac
            write(6, '(5x, 2a)') "new line list encountered,",
     &                           " reset nu and nucont"
            write(*, '(5x, 2a)') "new line list encountered,",
     &                           " reset nu and nucont"
            nu = 1
            nucont = 1
         end if
         ...
         nucont_loop: do
            if(wlvac .lt. cont_wave(nucont)) exit nucont_loop
            nucont = nucont + 1
         end do nucont_loop

         nu_loop: do
            if(wlvac .lt. waveset(nu)) exit nu_loop
            nu = nu + 1
            if(nu .gt. num_nu) exit file13_loop ! BOB HAS nu .ge. num_nu
         end do nu_loop

         do j = 8, ndepth, 8
            if_j(j+1) = .false.

!.... linecen=.026538/1.77245*gf*exp(-elo/hckt)*stim*xnfp/rho/dopple/freq
!.... congf = con_os * (wlvac / c_nm) * gf
!.... WHERE con_os = NORMALIZED CLASSICAL OSCILLATOR STRENGTH
!....              = PI * E^2 / (M_EL * C) / SQRT(PI)
!....                E IN ESU, M_EL IN G, C IN CM/S
!....              APPROX = 0.014974
!.... wlvac / c_nm = 1/freq
!.... xnfdop = xnfp / rho / dopple

            center = congf * xnfdop(j, nelion) * fastex(elo * hckt(j))

            if(center .ge. cont_tab(j, nucont)) then ! ABOVE THRESHOLD
               if_j(j+1) = .true.
               adamp = (gammar + gammas * xne(j) + gammaw * txnxn(j)) /
     &                 dopple(j, nelion)
               dopwave = dopple(j, nelion) * wlvac

!.... ORIGINAL linop DID adamp .le. 0.2 HERE AND STRONG DAMPING IN voigt
!.... NOW DO ALL adamp IN voigt

               iw = nu

               rw1: do ! RED WING
                  if(iw .gt. min(nu+100, num_nu)) exit rw1
                  cv = center *
     &                 voigt((waveset(iw) - wlvac)/dopwave, adamp)
                  xlines(j, iw) = xlines(j, iw) + cv
                  if(cv .lt. cont_tab(j, nucont)) exit rw1 !BELOW THRESHOLD
                  iw = iw + 1
               end do rw1
               ...
               bw1: do ! BLUE WING
                  if(iw .lt. max(nu-100, 1)) exit bw1
                  cv = center *
     &                 voigt((wlvac - waveset(iw))/dopwave, adamp)
                  xlines(j, iw) = xlines(j, iw) + cv
                  if(cv .lt. cont_tab(j, nucont)) exit bw1 !BELOW THRESHOLD
                  iw = iw - 1
               end do bw1

            end if ! CENTER .ge. TABCONT(J, NUCONT)

         end do ! J = 8, NDEPTH, 8

         do ki = 8, ndepth, 8

            if(if_j(ki-7) .or. if_j(ki+1)) then

               do j = ki-7, ki-1
                  center = congf * xnfdop(j, nelion) *
     &                     fastex(elo*hckt(j))

                  if(center .ge. cont_tab(j,nucont)) then
                     adamp = (gammar + gammas * xne(j) +
     &                        gammaw * txnxn(j)) / dopple(j,nelion)
                     dopwave = dopple(j,nelion) * wlvac
...
!.... NOW DO ALL adamp IN voigt
                     ...
                     rw2: do ! RED WING
                        if(iw .gt. min(nu+100, num_nu)) exit rw2
                        cv = center *
     &                       voigt((waveset(iw) - wlvac)/dopwave, adamp)
                        xlines(j, iw) = xlines(j, iw) + cv
                        if(cv .lt. cont_tab(j, nucont)) exit rw2 !.lt.THRESHOLD
                        iw = iw + 1
                     end do rw2
                     ...
                     bw2: do ! BLUE WING
                        if(iw .lt. max(nu-100, 1)) exit bw2
                        cv = center *
     &                       voigt((wlvac - waveset(iw))/dopwave, adamp)
                        xlines(j, iw) = xlines(j, iw) + cv
                        if(cv .lt. cont_tab(j, nucont)) exit bw2 !.lt.THRESHOLD
                        iw = iw - 1
                     end do bw2

                  end if ! CENTER .ge. CONT_TAB(J,NUCONT)

               end do ! J = KI-7, KI-1

            end if ! IF_J(KI-7) .OR. IF_J(KI+1)

         end do ! KI = 8, NDEPTH, 8

      end do file13_loop

      write(6, '(i10, f11.4, i7, 5es10.3)') line, wlvac, nelion, congf,
     &    elo, gammar, gammas, gammaw
      write(*, '(i10, f11.4, i7, 5es10.3)') line, wlvac, nelion, congf,
     &    elo, gammar, gammas, gammaw
...
!**************** E N D  S U B R O U T I N E  L I N O P ****************
      ...
      use atmosphere_parameters, only: ndepth
      use continuum_tables,      only: cont_tab, iwavecont
      use freq_set,              only: nu_first, nu_last, waveset
      use logtab                     ! ratiolg, tablog
      use physical_constants,    only: c_nm, pi4cnmi, con_os
      use temp_vars,             only: hckt
      use var_types
      use xnf_dop,               only: xnfdop
...
!------------------------- prelinop VARIABLES --------------------------
      ...
      integer(in_type) :: igs
      integer(in_type) :: igw
      integer(in_type) :: ios12
      integer(in_type) :: iwl
      integer(in_type) :: j
      integer(in_type) :: line
      integer(in_type) :: lineused
      integer(in_type) :: nelion
      integer(in_type) :: nucont = 1
      ...
      real(re_type)       :: gammas
      real(re_type)       :: gammaw
      real(re_type), save :: wl_last = 0.0d0
...
!------------------------- prelinop EXECUTION --------------------------
      ...
      write(6, '(/ a)')
     &   "prelinop - screening lines against the continuous opacity"
      write(*, '(/ a)')
     &   "prelinop - screening lines against the continuous opacity"
      write(6, '(/ 8a10 / )') "line", "iwl", "ielion", "ielo", "igflog",
     &                        "igr", "igs", "igw"

      wl_start = waveset(nu_first) - 1.0d0
      wl_stop = waveset(nu_last) + 1.0d0
      ...
      file12_loop: do ! READ THE WHOLE FILE OF SELECTED LINES
         read(12, iostat = ios12) iwl, ielion, ielo, igflog, igr, igs,
     &                            igw
         if(ios12 .ne. 0) exit file12_loop
         line = line + 1
         if(mod(line, 100000) .eq. 1) write(6, '(8i10)') line, iwl,
     &                              ielion, ielo, igflog, igr, igs, igw
         nelion = abs(ielion / 10)
         wlvac = exp(iwl * ratiolg) ! IN NM

         if(wlvac .lt. wl_last) then ! RESET FOR NEW LINE LIST
            write(6, '(2(5x, a, f12.4))') "last line =", wl_last,
     &                                    " this line =", wlvac
            write(*, '(2(5x, a, f12.4))') "last line =", wl_last,
     &                                    " this line =", wlvac
            write(6, '(5x, a)') "new line list, reset nucont"
            write(*, '(5x, a)') "new line list, reset nucont"
            nucont = 1
         end if
         ...
         nucont_loop: do
            if(iwl .lt. iwavecont(nucont)) exit nucont_loop
            nucont = nucont + 1
         end do nucont_loop

         if(wlvac .ge. wl_start .and. wlvac .le. wl_stop) then

!.... con_os = CLASSICAL OSCILLATOR STRENGTH
!....        = PI * E^2 / (M_EL * C) / SQRT(PI)
!....        WHERE E IN ESU, M_EL IN G, C IN CM/S
!....        APPROX = 0.014974
!.... wlvac / c_nm = 1/freq
!.... tablog(igflog) = gf

            congf = con_os * (wlvac / c_nm) * tablog(igflog)
            elo = tablog(ielo)

            keep_line = .false.
            j = 0

            line_loop: do ! TEST LINE CENTER AGAINST CONTINUOUS OPACITY
               j = j + 8
               if(j .gt. ndepth) exit line_loop

!.... center IS THE SAME AS kappa0 IN synthe
     ...
     &                  fastex(elo * hckt(j))
               ...
               if(center .ge. cont_tab(j, nucont)) then ! ABOVE THRESHOLD
                  wl4pi = wlvac * pi4cnmi
                  gammar = tablog(igr) * wl4pi
                  gammas = tablog(igs) * wl4pi
                  gammaw = tablog(igw) * wl4pi
                  keep_line = .true.
               end if

               if(keep_line) then
                  write(13) wlvac, nelion, congf, elo, gammar, gammas,
     &                      gammaw
                  lineused = lineused + 1
                  exit line_loop
               end if
      ...
      write(6, '(5x, a, i15, a, i15)') "lines screened =", line,
     &                                 ", lines kept =", lineused
      write(*, '(5x, a, i15, a, i15)') "lines screened =", line,
     &                                 ", lines kept =", lineused

      end subroutine prelinop

!************* E N D  S U B R O U T I N E  P R E L I N O P *************

      subroutine selectlines

!.... SELECT LINES FROM THE VARIOUS LINE LIST
!.... KEEP LINES THAT PASS THE LINE CENTER THRESHOLD FOR THIS MODEL'S
!.... TEMPERATURE STRUCTURE

!.... 2019 AUG - UPDATED TO FIORELLA CASTELLI'S GFORT VERSION
!.... 2017 JUL - CHANGED THE recl OF ALL FILES FROM BYTES TO 4-BYTE WORDS

      use atmosphere_parameters, only: ndepth
      use code_dimensions,       only: max_mion
      use continuum_tables           ! cont_tab, cont_wave, iwavecont
      use logtab                     ! ratiolg, tablog
      use physical_constants,    only: c_nm, pisqrt
      use temp_vars,             only: hckt
      use var_types
      use xnf_dop,               only: xnfdop
...
!------------------------ selectlines CONSTANTS ------------------------

!.... FIORELLA CASTELLI'S isox
      integer(in_type), parameter :: isox(46) = [
!....           H2           HD
     &          -0,       -4469, 
!....         12CH         13CH
     &        -005,       -1955,
...
!....        28SiH        29SiH        30SiH
     &        -035,       -1331,       -1516,
!....        40CaH        42CaH        43CaH        44CaH        46CaH
     &        -013,       -2189,       -2870,       -1681,       -4398,
!....        50CrH        52CrH        53CrH        54CrH
     &       -1362,        -077,       -1022,       -1626,
!....        54FeH        56FeH        57FeH        58FeH
     &       -1237,        -038,       -1658,       -2553,
!....       12C12C       12C13C       13C13C
     &    -005-005,   -005-1955,  -1955-1955,
!....       12C14N       13C14N       12C15N       13C15N
     &    -005-002,   -1955-002,   -005-2444,  -1955-2444,
!....       12C16O       13C16O       12C17O       13C17O       12C18O
     &    -005-001,   -1955-001,   -005-3398,  -1955-3398,   -005-2690,
!....       13C18O
     &  -1955-2690,
!.....     28Si16O      29Si16O      30Si16O      28Si18O
     &    -001-035,   -001-1331,   -001-1516,   -2690-035,
!....       51V16O
     &    -001-001 ]

!.... BOB KURUCZ'S isox
!!!!  integer(in_type), parameter :: isox(31) = [
!....           H2        12CH      13CH        14NH        15NH
!!!! &          -0,       -005,    -1955,       -002,      -2444,
!....         16OH        17OH      18OH       24MgH       25MgH
!!!! &        -001,      -3398,    -2690,       -105,       -996,
!....        26MgH       28SiH      29SiH      30SiH      12C12C
!!!! &        -947,       -035,     -1331,     -1516,   -005-005,
!....       12C13C      13C13C     12C14N     13C14N      12C15N
!!!! &   -005-1955, -1955-1955,  -005-002, -1955-002,  -005-2444,
!....       13C15N      23C16O     13C16O     12C17O      13C17O
!!!! &  -1955-2444  , -005-001, -1955-001, -005-3398, -1955-3398,
!....       12C18O      13C18O    28Si16O    29Si16O     30Si16O
!!!! &   -005-2690, -1955-2690,  -001-035, -001-1331,  -001-1516,
!....      28Ci18O
!!!! &   -2690-035 ]

!.... FIORELLA CASTELLI'S mol_codes
      integer(in_type), parameter :: mol_codes(46) = [
...
!.... BOB KURUCZ'S mol_codes
!!!!  integer(in_type), parameter :: mol_codes(31) = [
!!!! & 8410, 8460, 8461, 8470, 8471, 8480, 8481, 8482, 8510, 8511,
!!!! & 8512, 8530, 8531, 8532, 8680, 8681, 8682, 8690, 8691, 8692,
!!!! & 8693, 8700, 8701, 8702, 8703, 8704, 8705, 8890, 8891, 8892, 
!!!! & 8896 ]

!------------------------ selectlines VARIABLES ------------------------

      integer(int16) :: int2(6) !.... NOTE: 16 BIT INTEGER
      ...
      integer(in_type) :: igs
      integer(in_type) :: igw
      integer(in_type) :: imol
      integer(in_type) :: ios
      integer(in_type) :: irec
      integer(in_type) :: iso
      integer(in_type) :: iwl
      integer(in_type) :: j
      integer(in_type) :: kgflog
      integer(in_type) :: lenbytes
      integer(in_type) :: lenrec
      integer(in_type) :: mol_code
      integer(in_type) :: mol_code_old
      integer(in_type) :: n_12
      integer(in_type) :: n_diatomics = 0
      integer(in_type) :: n_h2o = 0
!!!!  integer(in_type) :: n_h3p = 0 ! COMMENT OUT BECAUSE CANNOT FIND H3+ FILE
      integer(in_type) :: n_highlines = 0
      integer(in_type) :: n_lowlines = 0
      integer(in_type) :: n_tio = 0
      integer(in_type) :: nelion
      integer(in_type) :: nu_cont

      logical :: first

      real(re_type) :: cenratio
      real(re_type) :: elo
      real(re_type) :: freq_cont
      real(re_type) :: gammar
      real(re_type) :: gr
      real(re_type) :: wlvac

!.... BOB KURUCZ USES xlines IN module_xlines_variable FOR xnfdopmax
!.... APPARENTLY JUST TO SAVE STORAGE

      real(re_type) :: xnfdopmax(max_mion, 344) = 0.0d0

!------------------------ selectlines EXECUTION ------------------------

      write(6, '(/a)') "selecting lines for this temperature structure"
      write(*, '(/a)') "selecting lines for this temperature structure"
      ...
      do nu_cont = 1, 344
         ...
         do nelion = 1, max_mion
            ...
            do j = 1, ndepth
               xnfdopmax(nelion, nu_cont) =
     &            max(xnfdopmax(nelion, nu_cont),
     &                xnfdop(j, nelion) / cont_tab(j, nu_cont))
            end do
!.... TEST THIS
!!!!        xnfdopmax(nelion, nu_cont) = 
!!!! &         maxval(xnfdop(1:ndepth, nelion) /
!!!! &                cont_tab(1:ndepth, nu_cont))
         end do

      end do

!!!!  write(*, '(a, f12.2 / )') "cont_wave(100) =", cont_wave(100)

!!!!  do nelion = 1, max_mion
!!!!     write(*, '(i5, es12.3)') nelion, xnfdopmax(nelion, 100)
!!!!  end do

!!!!  write(*, '(a)') "end of xnfdopmax at cont_wave(100)"

      n_12 = 0 ! COUNT THE NUMBER OF LINES WRITTEN TO UNIT 12

!.... LOWLINES = IONS 1 UP TO 4 OR 5

!.... DO NOT COMPILE WITH -assume byterecl

!!!!  lenbytes = in_type + 6 * i2_type ! 16 BYTES
      lenbytes = in_type + 6 * int16   ! 16 BYTES
      lenrec = lenbytes / 4 ! CONVERT TO 4-BYTE WORDS
      if(lenrec * 4 .lt. lenbytes) lenrec = lenrec + 1

      open(unit = 11, file = 'lowlines_file', status = 'old',
     ...
     &     recl = lenrec)
      ...
      freq_cont = 0.0d0
      nu_cont = 1
      n_lowlines = 0 ! COUNT THE NUMBER OF LOWLINES USED
      irec = 0

      do ! READ THE LOWLINES FILE
         irec = irec + 1
         read(11, rec = irec, iostat = ios) iwl, int2(1:6)
         if(ios .ne. 0) exit
         ielion = int2(1)
         ielo = int2(2)
         igflog = int2(3)
         ...
         do
            if(iwl .lt. iwavecont(nu_cont)) exit
            freq_cont = c_nm / cont_wave(nu_cont)
            nu_cont = nu_cont + 1
         end do

         if(freq_cont .gt. 0.0d0 .and.
     &      xnfdopmax(nelion, nu_cont) .gt. tiny(1.0)) then

!.... linecen=.026538/1.77245*gf*exp(-elo/hckt)*stim*xnfp/rho/dopple/freq
!.... wlvac=exp(iwl*ratiolg)
!.... gf=tablog(igflog)

!!!!        cenratio = 0.026538d0 / 1.77245d0 * tablog(igflog) *
            cenratio = 0.026538d0 / pisqrt    * tablog(igflog) *
     &                 xnfdopmax(nelion, nu_cont) / freq_cont

            if(cenratio .ge. 1.0d0 .and.
     &         cenratio * exp(-tablog(ielo)*hckt(ndepth)).ge.1.0d0) then
               n_12 = n_12 + 1
               n_lowlines = n_lowlines + 1
               write(12) iwl, ielion, ielo, igflog, igr, igs, igw

               if(first) then
                  first = .false.
                  wlvac = exp(iwl * ratiolg)
                  write(6, '(5x, 2a, f12.4)') "first record",
     &                  " wavelength(nm) =", wlvac
                  write(*, '(5x, 2a, f12.4)') "first record",
     &                  " wavelength(nm) =", wlvac
                  write(6, '( / 8a10 / )') "rec#", "iwl", "ielion",
     &                  "ielo", "igflog", "igr", "igs", "igw"
                  write(6, '(8i10)') n_lowlines, iwl, ielion, ielo,
     &                               igflog, igr, igs, igw
               end if

               if(mod(n_lowlines, 100000) .eq. 1) write(6, '(8i10)')
     &            n_lowlines, iwl, ielion, ielo, igflog, igr, igs, igw
            end if

         end if

      end do ! LOWLINES FILE

      if(n_lowlines .gt. 0) then
         write(6, '(8i10)') n_lowlines, iwl, ielion, ielo, igflog,
     &                      igr, igs, igw
         write(*, '(5x, a, i11, a, f12.4)') "last record", irec-1,
     &       "  wavelength(nm) =", exp(iwl*ratiolg)
      end if

      write(6, '(i16, a)') n_lowlines, " low lines selected"
      write(*, '(i16, a)') n_lowlines, " low lines selected"

      close(unit = 11)

!.... HIGHLINES = IONS 4 OR 5 TO 9
...
!!!!  lenbytes = in_type + 6 * i2_type ! 16 BYTES
      ...
      open(unit = 21, file = 'highlines_file', status = 'old',
     &     form = 'unformatted', action = 'read', access = 'direct',
     &     recl = lenrec)
      ...
      freq_cont = 0.0d0
      nu_cont = 1
      n_highlines = 0 ! COUNT THE NUMBER OF HIGHLINES USED
      irec = 0

      do ! READ THE HIGHLINES FILE
         irec = irec + 1
         read(21, rec = irec, iostat = ios) iwl, int2(1:6)
         if(ios .ne. 0) exit
         ielion = int2(1)
         ielo = int2(2)
         igflog = int2(3)
         ...
         if(freq_cont .gt. 0.0d0 .and.
!!!! &      xnfdopmax(nelion, nu_cont) .gt. 0.0d0)) then
     &      xnfdopmax(nelion, nu_cont) .gt. tiny(1.0)) then ! SAME AS LOWLINES
!!!!        cenratio = 0.026538d0 / 1.77245d0 * tablog(igflog) *
            cenratio = 0.026538d0 / pisqrt    * tablog(igflog) *
     &                 xnfdopmax(nelion, nu_cont) / freq_cont
     ...
     &                  " wavelength(nm) =", wlvac
                  write(6, '( / 8a10 / )') "rec#", "iwl", "ielion", 
     &                  "ielo", "igflog", "igr", "igs", "igw"
                  write(6, '(8i10)') n_highlines, iwl, ielion, ielo,
     &                               igflog, igr, igs, igw
               end if

               if(mod(n_highlines, 100000) .eq. 1) write(6, '(8i10)')
     &            n_highlines, iwl, ielion, ielo, igflog, igr, igs, igw
            end if

         end if

      end do ! HIGHLINES FILE

      if(n_highlines .gt. 0) then
         write(6, '(8i10)') n_highlines, iwl, ielion, ielo, igflog,
     &                      igr, igs, igw
         write(*, '(5x, a, i11, a, f12.4)') "last record", irec-1,
     &       "  wavelength(nm) =", exp(iwl*ratiolg)
      end if

      write(6, '(i16, a)') n_highlines, " high lines selected"
      write(*, '(i16, a)') n_highlines, " high lines selected"
...
!!!!  lenbytes = in_type + 6 * i2_type ! 16 BYTES
      ...
      open(unit = 31, file = 'diatomics_file', status = 'old', 
     &     form = 'unformatted', action = 'read', access = 'direct',
     &     recl = lenrec)

      write(6, '(/ a )') "diatomic molecules"
      ...
      do ! READ THE DIATOMICS FILE
         irec = irec + 1
         read(31, rec = irec, iostat = ios) iwl, int2(1:6)
         if(ios .ne. 0) exit
         ielion = int2(1)
         ielo = int2(2)
         igflog = int2(3)
         ...
         kgflog = igflog
         mol_code = abs(ielion)
         nelion = abs(ielion/10) ! MOVED UP HERE
...
!.... THE BAND NUMBER IS STORED IN igs SINCE WE IGNORE STARK BROADENING
         igs = 1

         if(mol_code .ne. mol_code_old) then
            mol_code_old = mol_code
            imol = 1

            do
               if(mol_code .eq. mol_codes(imol)) exit
               imol = imol + 1

!!!!           if(imol .gt. 31) then ! BOB KURUCZ'S DIMENSION
               if(imol .gt. 46) then ! FIORELLA CASTELLI'S DIMENSION
                  write(6, '(a)') "unknown mol_code"
                  write(*, '(a)') "unknown mol_code"
                  write(6, '(2i9, i12, 6i6)') irec, mol_code, iwl,
     &               ielion, igflog, ielo, igr, igs, igw
                  write(*, '(2i9, i12, 6i6)') irec, mol_code, iwl,
     &               ielion, igflog, ielo, igr, igs, igw
                  stop
               end if
...
!!!!        cenratio = 0.026538d0 / 1.77245d0 * tablog(igflog) *
     ...
     &               " wavelength(nm) =", wlvac
                  write(6, '( / 8a10 / )') "rec#", "iwl", "ielion",
     &               "ielo", "igflog", "igr", "igs", "igw"
                  write(6, '(8i10)') n_diatomics, iwl, ielion, ielo,
     &                               igflog, igr, igs, igw
               end if

               if(mod(n_diatomics, 100000) .eq. 1) write(6, '(8i10)')
     &            n_diatomics, iwl, ielion, ielo, igflog, igr, igs, igw
            end if

         end if

      end do ! DIATOMICS FILE

      if(n_diatomics .gt. 0) then
         write(6, '(8i10)') n_diatomics, iwl, ielion, ielo, igflog,
     &                      igr, igs, igw
         write(*, '(5x, a, i11, a, f12.4)') "last record", irec-1,
     &      "  wavelength(nm) =", exp(iwl*ratiolg)
      end if

      write(6, '(i16, a)') n_diatomics, " diatomics lines selected"
      write(*, '(i16, a)') n_diatomics, " diatomics lines selected"

      close(unit = 31)

!.... TIO MOLECULES - THIS IS TIOSCHWENKE.BIN
...
!!!!  lenbytes = in_type + 6 * i2_type ! 16 BYTES
      ...
      open(unit = 41, file = 'tiolines_file', status = 'old', 
     &     form = 'unformatted', action = 'read', access = 'direct',
     &     recl = lenrec)

      write(6, '(/ a )') "TiO molecules"
      write(*, '(/ a )') "TiO molecules"
      ...
      do ! READ THE TIO FILE
         irec = irec + 1
         read(41, rec = irec, iostat = ios) iwl, int2(1:6)
         if(ios .ne. 0) exit
         ielion = int2(1)
         ielo = int2(2)
         igflog = int2(3)
         ...
         kgflog = igflog
         iso = abs(ielion) ! RANGES FROM 8950 TO 8954

         if(iso .eq. 8950) then      ! 46TiO
            igflog = max(kgflog - 1101, 1)

         else if(iso .eq. 8951) then ! 47TiO
            igflog = max(kgflog - 1138, 1)

         else if(iso .eq. 8952) then ! 48TiO
            igflog = max(kgflog - 131, 1)

         else if(iso .eq. 8953) then ! 49TiO
            igflog = max(kgflog - 1259, 1)

         else if(iso .eq. 8954) then ! 50TiO
            igflog = max(kgflog - 1272, 1)

         else
            write(*, '(a / i9, i12, 6i6)') "tio error", irec,
     &         iwl, ielion, igflog, ielo, igr, igs, igw
            stop
         end if
...
!!!!        cenratio = 0.026538d0 / 1.77245d0 * tablog(igflog) *
     ...
     &               " wavelength(nm) =", wlvac
                  write(6, '( / 8a10 / )') "rec#", "iwl", "ielion",
     &               "ielo", "igflog", "igr", "igs", "igw"
                  write(6, '(8i10)') n_tio, iwl, ielion, ielo, igflog,
     &                               igr, igs, igw
               end if

               if(mod(n_tio, 100000) .eq. 1) write(6, '(8i10)') n_tio,
     &            iwl, ielion, ielo, igflog, igr, igs, igw
            end if
      ...
      if(n_tio .gt. 0) then
         write(6, '(8i10)') n_tio, iwl, ielion, ielo, igflog, igr, igs,
     &                      igw
         write(*, '(5x, a, i11, a, f12.4)') "last record", irec-1,
     &       "  wavelength(nm) =", exp(iwl*ratiolg)
      end if

      write(6, '(i16, a)') n_tio, " TiO lines selected"
      write(*, '(i16, a)') n_tio, " TiO lines selected"

      close(unit = 41)

!.... H2O

!!!!  lenbytes = in_type + 2 * i2_type ! 8 BYTES
      lenbytes = in_type + 2 * int16   ! 8 BYTES
      lenrec = lenbytes / 4 ! CONVERT TO 4-BYTE WORDS
      if(lenrec * 4 .lt. lenbytes) lenrec = lenrec + 1

      open(unit = 51, file = 'h2o_file', status = 'old', 
     &     form = 'unformatted', action = 'read', access = 'direct',
     &     recl = lenrec)
      ...
      do ! READ THE H2O FILE
         irec = irec + 1
         read(51, rec = irec, iostat = ios) iwl, int2(1:2)
         if(ios .ne. 0) exit
         ielo = int2(1)
         igflog = int2(2)
         ...
         do
            if(iwl .lt. iwavecont(nu_cont)) exit
!.... freq_cont IS ALWAYS DEFINED BECAUSE THE FIRST H2O LINE = 408 NM
            freq_cont = c_nm / cont_wave(nu_cont)
            gammar = 2.474d-22 * freq_cont**2 * 0.001d0
            gr = log10(gammar)
            igr = gr * 1000.0d0 + 16384.5d0
            nu_cont = nu_cont + 1
         end do
         ...
         if(iso .eq. 1) then
            igflog = max(kgflog-001, 1)  ! 1H1H16O
         else if(iso .eq. 2) then
            igflog = max(kgflog-3398, 1) ! 1H1H17O
         else if(iso .eq. 3) then
            igflog = max(kgflog-2690, 1) ! 1H1H18O
         else if(iso .eq. 4) then
            igflog = max(kgflog-5000, 1) ! 1H2H16O
         end if
...
!!!!        cenratio = 0.026538d0 / 1.77245d0 * tablog(igflog) *
     ...
     &         cenratio * exp(-tablog(ielo)*hckt(ndepth)).ge.1.0d0) then
               n_12 = n_12 + 1
               n_h2o = n_h2o + 1
               ielo = log10(elo) * 1000.0d0 + 16384.5d0
               igs = 1    ! GAMMAS = 0.
               igw = 9384 ! LOG GAMMAW=-7
               write(12) iwl, ielion, ielo, igflog, igr, igs, igw
               ...
               if(first) then
                  first = .false.
                  wlvac = exp(iwl * ratiolg)
                  write(6, '(5x, a, a28, f12.4)') "first record",
     &                  " wavelength(nm) =", wlvac
                  write(*, '(5x, a, a28, f12.4)') "first record",
     &                  " wavelength(nm) =", wlvac
                  write(6, '( / 8a10 / )') "rec#", "iwl", "ielion", 
     &                  "ielo", "igflog", "igr", "igs", "igw"
                  write(6, '(8i10)') n_h2o, iwl, ielion, ielo, igflog,
     &                               igr, igs, igw
               end if

               if(mod(n_h2o, 100000) .eq. 1) write(6, '(8i10)') n_h2o,
     &            iwl, ielion, ielo, igflog, igr, igs, igw
            end if
      ...
      if(n_h2o .gt. 0) then
         write(6, '(8i10)') n_h2o, iwl, ielion, ielo, igflog, igr, igs,
     &                      igw
         write(*, '(5x, a, i11, a, f12.4)') "last record", irec-1,
     &       "  wavelength(nm) =", exp(iwl*ratiolg)
      end if

      write(6, '(i16, a)') n_h2o, " H2O lines selected"
      write(*, '(i16, a)') n_h2o, " H2O lines selected"

      close(unit = 51)

!.... H3+
!.... COMMENT OUT FOR NOW.  CANNOT FIND H3PLUS FILE
...
!!!!  open(unit = 61, file = 'h3plus_file', status = 'old', 
!!!! &     form = 'unformatted', action = 'read', access = 'direct',
!!!! &     recl = lenrec)

!!!!  write(6, '(/ a )') "H3+ molecule"
!!!!  write(*, '(/ a )') "H3+ molecule"
...
!!!!  do ! READ THE H3+ FILE
!!!!     irec = irec + 1
!!!!     read(61, rec = irec, iostat = ios) iwl, int2(1:6)
!!!!     if(ios .ne. 0) exit
!!!!     ielion = int2(1)
!!!!     ielo = int2(2)
!!!!     igflog = int2(3)
...
!!!!              write(6, '( / 8a10 / )') "rec#", "iwl", "ielion", 
!!!! &                  "ielo", "igflog", "igr", "igs", "igw"
!!!!              write(6, '(8i10)') n_h3p, iwl, ielion, ielo, igflog,
!!!! &                               igr, igs, igw
!!!!           end if

!!!!           if(mod(n_h3p, 100000) .eq. 1) write(6, '(8i10)') n_h3p,
!!!! &            iwl, ielion, ielo, igflog, igr, igs, igw
!!!!        end if
...
!!!!  if(n_h3p .gt. 0) then
!!!!     write(6, '(8i10)') n_h3p, iwl, ielion, ielo, igflog, igr, igs,
!!!! &                      igw
!!!!     write(*, '(5x, a, i11, a, f10.4)') "last record", irec-1,
!!!! &       "  wavelength(nm) =", exp(iwl*ratiolg)
!!!!  end if

!!!!  write(6, '(i16, a)') n_h3p, " H3+ lines selected"
!!!!  write(*, '(i16, a)') n_h3p, " H3+ lines selected"

!!!!  close(unit=61)

      write(*, '(/ a, i12)') "total number of lines selected =", n_12

      end subroutine selectlines

!********** E N D  S U B R O U T I N E  S E L E C T L I N E S **********

      function vcse1f(x) result(vcse1f_)
...
!------------------------------ vcse1f EXECUTION -----------------------
      ...
      if(x .gt. 0.0d0) then
         ...
         if(x .le. 0.01d0) then
            vcse1f_ = -log(x) - 0.577215d0 + x

         else if(x .le. 1.0d0) then
            vcse1f_ = -log(x) - 0.57721566d0 +
     &                          x * ( 0.99999193d0 +
     &                          x * (-0.24991055d0 +
     &                          x * ( 0.05519968d0 +
     &                          x * (-0.00976004d0 +
     &                          x *  0.00107857d0))))
            ...
            vcse1f_ = (x * (x + 2.334733d0) + 0.25062d0) /
     &                (x * (x + 3.330657d0) + 1.681534d0) /
     &                x * fastex(x)
         end if
...
!*************** E N D  F U N C T I O N  V C S E 1 F  ******************

      function voigt(v, a) result(voigt_out)

!.... BOB'S ORIGINAL FAST VOIGT FUNCTION
...
!--------------------------- voigt ARGUMENTS ---------------------------
      ...
      real(re_type), intent(in) :: a ! = GAMMA/(4 PI DELTA NU DOPPLER)
      real(re_type), intent(in) :: v ! = (DELTA NU)/(DELTA NU DOPPLER)
      real(re_type)             :: voigt_out
...
!.... USING "d0" FOR tabh1 CAUSES A DIFFERENCE
...
!!!!     forall(i = 0:2000) h0tab(i) = real(i, re_type) * vinv
         ...
         m1 = map_cs(tabvi(0:80), tabh1(0:80),
     &               h0tab(0:2000), h1tab(0:2000))
      ...
      iv = nint(v * vsteps, in_type)
      vv = v * v

      if(a .lt. 0.2d0) then

         if(v .gt. 10.0d0) then
            voigt_out = 0.5642d0 * a / vv
         else
            voigt_out = h0tab(iv) + a * (h1tab(iv) + a * h2tab(iv))
         end if

      else if(a .gt. 1.4d0 .or. a+v .gt. 3.2d0) then
         aa = a * a
         u =(aa + vv) * sqrt(2.0d0)
         voigt_out = a * 0.79788d0 / u

         if(a .le. 100.0d0) then
            aau = aa / u
            vvu = vv / u
            uu = u * u
            voigt_out = voigt_out * (1.0d0 + (((aau - 10.0d0 * vvu) *
     &                                          aau * 3.0d0 + 
     &                                          15.0d0 * vvu * vvu) +
     &                                          3.0d0 * vv - aa) / uu)
         end if
     ...
     &          h0tab(iv) * 0.66667d0 * vv * vv
         ...
         voigt_out = (h0tab(iv) + a * (hh1 + a * (hh2 + a * (hh3 + 
     &                            a * hh4)))) *
...
!************* E N D   F U N C T I O N   V O I G T _ K U R *************

      subroutine xlinop

!.... 2014 APR - CHANGED n TO nblo AND m TO nbup
!.... 2009 AUG - DEFINED dopph HERE, LIKE SYNTHE
!.... 2007 SEP - THIS IS NOW READ BY DEFAULT, if_op(17) .eq. .true.
!....          - FIXED PROBLEM WITH FAR IR OF HYDROGEN LINES
...
!--------------------------- INTERFACE BLOCK ---------------------------
      ...
      interface
         ...
         function hprof4(j, nblo, nbup, delw, dopph) result(h_prof4)
         use var_types
         integer(in_type), intent(in) :: j
         integer(in_type), intent(in) :: nblo
         integer(in_type), intent(in) :: nbup
         real(re_type),    intent(in) :: delw
...
!-------------------------- xlinop VARIABLES ---------------------------
      ...
      integer(in_type) :: i
      integer(in_type) :: ifj(max_d+1)
      integer(in_type) :: ios19
      integer(in_type) :: iw
      integer(in_type) :: iwl
      integer(in_type) :: j
      integer(in_type) :: ki
...
!!!!  integer(in_type) :: n ! LOCAL IN do concurrent
      integer(in_type) :: n_auto
      integer(in_type) :: n_cor
      integer(in_type) :: n_hyd
      integer(in_type) :: n_norm
      integer(in_type) :: n_merg
      integer(in_type) :: n_prd
      ...
      integer(in_type) :: nu
      integer(in_type) :: nucont
      integer(in_type) :: rec19

      logical, save :: first = .true.

      real(re_type)       :: adamp
      real(re_type)       :: ashore
!!!!  real(re_type)       :: bolth(max_d, 10)  ! ORIGINAL
      real(re_type)       :: bolth(max_d, 100) ! FIORELLA
      real(re_type)       :: bshore
      real(re_type)       :: center
      real(re_type)       :: con
      real(re_type)       :: congf
      real(re_type), save :: cont_x(25, 17)
      real(re_type)       :: cv
      real(re_type)       :: dopwave

!.... HYDROGEN EXCITATION ENERGIES IN CM-1

      real(re_type), save :: e_hyd(100) ! FIORELLA
      real(re_type)       :: elo
      real(re_type)       :: epsil
      real(re_type)       :: frelin
      real(re_type)       :: g
...
!.... IONZATION ENERGIES IN CM-1 

      data cont_x(1:25, 1) /  ! H I = 1.00
     ...
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 7) /  ! Mg II = 12.01
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     ...
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 9) /  ! Al II = 13.01
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 10) / ! Si I = 14.00
     ...
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 11) / ! Si II = 14.01
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 12) / ! Ca I = 20.00
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 13) / ! Ca II = 20.01
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     ...
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0/
      ...
      data cont_x(1:25, 15) / ! Na I = 11.00
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
     &      0.0d0,       0.0d0,       0.0d0,       0.0d0,       0.0d0,
...
!-------------------------- xlinop EXECUTION ---------------------------
      ...
      if(first) then
         ...
         e_hyd(9) = 108324.720d0
         e_hyd(10) = 108581.988d0

!.... FIORELLA DOES THE VALUES UP TO 100

!.... REPLACED 2019 APR
!!!!     forall(n = 11:100) e_hyd(n) = hyd_ip -
!!!! &                                 ryd_hyd / real(n * n, re_type)

         do concurrent(integer(in_type) :: n = 11:100)
            e_hyd(n) = hyd_ip - ryd_hyd / real(n * n, re_type)
         end do

      end if

      write(6, '(/ a / a10, a11, a9, a8, 4a10)') "xlinop", "line",
     &   "wave(nm)", "congf", "type", "elo", "gammar", "gammas","gammaw"
      write(*, '(/ a / a10, a11, a9, a8, 4a10)') "xlinop", "line",
     &   "wave(nm)", "congf", "type", "elo", "gammar", "gammas","gammaw"

!.... txnxn HAS BEEN MOVED TO main TO BE AVAILABLE FOR SEVERAL ROUTINES

      do j = 1, ndepth
!!!!     bolth(j, 1:10) = exp(-e_hyd(1:10) * hckt(j)) * xnfdop(j, 1)
         bolth(j, 1:100) = exp(-e_hyd(1:100) * hckt(j)) * xnfdop(j, 1)

!.... BOB USES nstark, THEN SETS merge_n = nstark, BOTH REAL

         merge_n = 1600.0d0 / xne(j)**p2_15 ! BASICALLY INGLIS-TELLER
!!!!     merge_e(j) = 109737.312d0/ merge_n**2 ! CM-1
         merge_e(j) = rydbg / merge_n**2       ! CM-1
      end do

      line = 0

!.... NOTE: INITIALIZING nu AND nucont ONCE ASSUMES THAT THE LINES IN
!....       FILE 19 ARE IN ORDER OF INCREASING wlvac
!....       THE FILE gfallnlte.dat HAS BEEN TESTED AND FOUND TO BE IN 
!....       ORDER OF INCREASING WAVELENGTH
      ...
      file19_loop: do
         ...
         read(19, rec = rec19, iostat = ios19) wlvac, elo, congf, 
     &      nblo, nbup, nelion, ltype, ncon, nelionx, gammar, gammas,
     &      gammaw, iwl
         if(ios19 .ne. 0) exit file19_loop
         line = line + 1

         if(mod(line, 10000) .eq. 1) then
            write(6, '(i10, f11.4, es12.3, i5, f12.3, 3es10.3)') 
     &         line, wlvac, congf, ltype, elo, gammar, gammas, gammaw
            write(*, '(i10, f11.4, es11.3, i5, f12.3, 3es10.3)')
     &         line, wlvac, congf, ltype, elo, gammar, gammas, gammaw
         end if

         do
            if(iwl .lt. iwavecont(nucont)) exit
            nucont = nucont + 1
         end do

         do
            if(wlvac .lt. waveset(nu)) exit
            nu = nu + 1
            if(nu .gt. num_nu) exit file19_loop
         end do

         if(ltype .eq. -1 .and.        ! HYDROGEN LINE
     &      wlvac .le. 20000.0d0) then ! UPPER LIMIT AVOIDS PROBLEM BOB
                                       ! FOUND WITH FAR-IR H LINES
            n_hyd = n_hyd + 1          ! COUNT THE HYDROGEN LINES

            do j = 1, ndepth
               center = congf * bolth(j, nblo)

               if(center .ge. cont_tab(j, nucont)) then
                  wcon = 1.0d7 / (cont_x(ncon, 1) - merge_e(j)) ! NM
                  iw = nu

                  do ! LINE CENTER AND RED WING
                     if(iw .gt. min(nu + 2000, num_nu)) exit

                     if(waveset(iw) .ge. wcon) then
                        cv = center * hprof4(j, nblo, nbup,
     &                                       (waveset(iw)-wlvac),
     &                                       dopple(j, 1))
                        xlines(j, iw) = xlines(j, iw) + cv
                        if(cv .lt. cont_tab(j, nucont)) exit
                     end if

                     iw = iw + 1
                  end do ! LINE CENTER AND RED WING

                  iw = nu - 1 ! RESET TO 1 BLUE OF LINE CENTER

                  do ! BLUE WING

                     if(iw .lt. 1) then ! THIS MUST BE TESTED SEPARATELY
                        exit
                     else if(waveset(iw) .lt. wcon) then ! TEST SEPARATELY
                        exit
                     else
                        cv = center * hprof4(j, nblo, nbup,
     &                                       (waveset(iw)-wlvac),
     &                                       dopple(j, 1))
         ...
         else if(ltype .eq. 0 .or.  ! NORMAL LINE
     &           ltype .eq. 3) then ! PRD LINE
            if(ltype .eq. 0) n_norm = n_norm + 1
            if(ltype .eq. 3) n_prd = n_prd + 1

!!!!        if(mod(line, 1000) .eq. 1) write(*, '(2i10, 2f12.6, 5es12.3,
!!!! &                                          i10)')
!!!! &         line, nu, wlvac, waveset(nu), congf, elo, gammar, gammas,
!!!! &         gammaw, nelion
     ...
     &                     gammaw * txnxn(j)) / dopple(j, nelion)
                  ...
                  if(ncon .gt. 0) wcon = 1.0d7 /
     &                                (cont_x(ncon,nelionx) -merge_e(j))

                  if(wlvac .ge. wcon) then

!.... BOB TESTS adamp .gt. 0.2 HERE, BUT THAT IS NOW DONE IN voigt

                     iw = nu

                     do ! CENTER AND RED WING
                        if(iw .gt. min(nu + 2000, num_nu)) exit
                        cv = center * voigt((waveset(iw) - wlvac) /
     &                                      dopwave, adamp)
                        xlines(j, iw) = xlines(j, iw) + cv
                        if(cv .lt. cont_tab(j, nucont)) exit
                        iw = iw + 1
                     ...
                     do ! BLUE WING
                        ...
                        if(iw .lt. 1) then ! THIS MUST BE TESTED SEPARATELY
                           ...
                           cv = center * voigt((wlvac - waveset(iw)) /
     &                                         dopwave, adamp)
                           xlines(j, iw) = xlines(j, iw) + cv
                           if(cv .lt. cont_tab(j, nucont)) exit
                           iw = iw - 1
            ...
            do ki = 8, ndepth, 8
               ...
               if(ifj(ki - 7) + ifj(ki + 1) .gt. 0) then
     ...
     &                                            dopwave, adamp)
                           ...
                           do ! BLUE WING
                              ...
                              if(iw .lt. 1) then ! THIS MUST BE TESTED SEPARATELY
                                 exit
                              else if(waveset(iw) .lt. wcon) then ! SEPARATE TEST
                                 exit
                              else
                                 cv = center * 
     &                                voigt((wlvac-waveset(iw)) /
     &                                      dopwave, adamp)
                                 xlines(j, iw) = xlines(j, iw) + cv
                                 if(cv .lt. cont_tab(j, nucont)) exit
                                 iw = iw - 1
         ...
         else if(ltype .eq. 1) then ! AUTOIONIZING LINE
            n_auto = n_auto + 1
            ashore = gammas ! FOR THESE LINES ashore IS STORED IN gammas
            bshore = gammaw ! FOR THESE LINES bshore IS STORED IN gammaw
            frelin = c_nm / wlvac
            g = congf ! FOR THESE LINES g IS STORED IN congf

            do j = 1, ndepth
               center = bshore * g * xnfp(j, nelion) * rhoinv(j) *
     &                  fastex(elo * hckt(j))
               if(center .ge. cont_tab(j,nucont)) then
                  iw = nu
                  ...
                  do ! CENTER AND RED WING
                     if(iw .gt. min(nu + 2000, num_nu)) exit
                     epsil = 2.0d0 * (c_nm / waveset(iw)-frelin)/gammar
                     cv = center * (ashore*epsil+bshore) /
     &                             (epsil**2 + 1.0d0) / bshore
                  ...
                  do ! BLUE WING
                     if(iw .lt. max(nu - i, 1)) exit
                     epsil = 2.0d0 * (c_nm / waveset(iw) - frelin) /
     &                       gammar
                     cv = center * (ashore * epsil+bshore) /
     &                             (epsil**2 + 1.0d0) / bshore
                     xlines(j,iw)=xlines(j,iw)+cv
         ...
         else if(ltype .eq. 2) then ! CORONAL LINE
            n_cor = n_cor + 1
            continue

         else ! MERGED CONTINUUM
            n_merg = n_merg + 1
            nlast = ltype ! FOR THIS LINE nlast IS STORED IN ltype
            z = 1.0d0
            if(nelion .eq. 4) z = 2.0d0
            wshift = 1.0d7 / (1.0d7 / wlvac - 109737.312d0 * z**2 /
     &               real(nlast**2, re_type))
            xsectg = congf

            do j = 1, ndepth
               merge_w = 1.0d7 / (1.0d7 / wlvac - merge_e(j) * z**2)
               wmax = max(merge_w, wshift)
               con = xsectg * xnfp(j, nelion) * fastex(elo * hckt(j)) *
     &               rhoinv(j)
               iw = nu

               do
                  if(iw .gt. min(nu + 1000, num_nu)) exit
      ...
      write(6, '(i10, f11.4, es12.3, i5, f12.3, 3es10.3)') line, wlvac,
     &   congf, ltype, elo, gammar, gammas, gammaw
      write(*, '(i10, f11.4, es11.3, i5, f12.3, 3es10.3)') line, wlvac,
     &   congf, ltype, elo, gammar, gammas, gammaw

      write(6, *)
      write(*, *)
      write(6, '(i6, a)') n_auto, " auto-ionizing lines"
      write(*, '(i6, a)') n_auto, " auto-ionizing lines"
      write(6, '(i6, a)') n_cor,  " coronal lines"
      write(*, '(i6, a)') n_cor,  " coronal lines"
      write(6, '(i6, a)') n_hyd,  " hydrogen lines"
      write(*, '(i6, a)') n_hyd,  " hydrogen lines"
      write(6, '(i6, a)') n_merg, " merged lines"
      write(*, '(i6, a)') n_merg, " merged lines"
      write(6, '(i6, a)') n_norm, " normal lines"
      write(*, '(i6, a)') n_norm, " normal lines"
      write(6, '(i6, a)') n_prd,  " prd lines - included with normal"
      write(*, '(i6, a)') n_prd,  " prd lines - included with normal"
