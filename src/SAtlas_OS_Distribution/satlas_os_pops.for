...
!.... 2015 JUN - USE abund_def = DEFAULT ABUNDANCES
!....            USE abund IN THE CODE AFTER ANY CHANGES/SCALING
!.... 2010 JAN - REMOVED TEST ON if_pres
...
!.... 2015 JUN - USE abund_def = DEFAULT ABUNDANCES
!....            USE abund IN THE CODE AFTER ANY CHANGES/SCALING
!.... 2010 JAN - REMOVED SECTION FOR READING IN POPULATIONS
...
!.... 2015 JUN - USE abund_def = DEFAULT ABUNDANCES
!....            USE abund IN THE CODE AFTER ANY CHANGES/SCALING
!.... 2014 APR - RENAMED rhox TO rhodz FOR RHO * DZ
!.... 2007 MAR - USE ndepth IN PLACE OF nrhox OR ntau
...
!.... if(j.gt.1)xne(j)=xne(j-1)*p_gas(j)/p_gas(j-1)
     ...
     &                  t73, a, t82, a)')
     &   "in NELECT:",
     &   "tauros", "temp", "p_gas", "xne", "xnatom", "wtmole", "rho", 
     &   "chargesq"

      write(6, '(i3, es11.3, f10.1, 6es11.3)')
     &   (j, tauros(j), t(j), p_gas(j), xne(j), xnatom(j), wtmole(j),
...
!.... USING abund(id, 1) IS THE SAME AS USING abund(id, j)
!!!!! THIS IS THE ONLY USE OF abund IN nmolec

      do ki = 2, n_equa
         id = id_equa(ki)
         if(id .lt. 100) xab(ki) = max(abund(id, 1), 1.0d-20) ! ATLAS12
      end do
...
!.... 2005 NOV 09 - NEW TEMPERATURE CUTOFF @ 10000 K
     ...
     &         "tauros", "temp", "p_gas", "xne", "xnatom", "rho"
            write(6, '( (i3, es11.3, f10.1, 4es11.3) )') 
     &          (j, tauros(j), t(j), p_gas(j), xne(j), xnatom(j),rho(j),
     &           j = 1, ndepth )
            jmol1 = 1
