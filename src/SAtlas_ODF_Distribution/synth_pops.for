...
!.... POPULATION ROUTINES FOR SPECTRUM SYNTHESIS
!.... BUILT FROM VERSION OF ATLAS7 2005JUN22
!               - PFSAHA DATA FOR 41.02
!     BUT ADDING SOME NOTATION AND CODE FROM ATLAS_ODF/OS
...
!.... 2014 MAY- CHANGED module_atlas_dimensions TO module_code_dimensions
!.... 2010 FEB - REMOVED TEST ON if_pres
      ...
      use synth_xnmol_vars,      only: xn_mol, xnfp_mol
...
!--------------------------- pops EXECUTION ----------------------------
      ...
      if(if_mol) then ! WITH MOLECULES
         ...
         if(code .ge. 100.0d0) then ! DO THE MOLECULES
            jmol = minloc(abs(molcode(1:nummol) - code), DIM=1)

            if(molcode(jmol) .ne. code) then
               number(1:ndepth, 1) = 0.0d0
            else

               if(mode .eq. 1) then
                  number(1:ndepth, 1) = xnfp_mol(1:ndepth, jmol)
                  ...
                  number(1:ndepth, 1) = xn_mol(1:ndepth, jmol) /
     &                                  xnfp_mol(1:ndepth, jmol)
               end if

            end if

         else if(code .gt. 0.0d0) then ! ATOMS WITH THE MOLECULES
            c = code

            if(mode .lt. 10) then
               ion = 1
            else
               ion = (c - real(int(c, in_type), re_type)) * 100.0d0 +
     &               1.5d0
            end if
     ...
     &            molcode(jmol) .lt. c + 0.001d0) then
                  ...
                  if(mode .eq. 1 .or. mode .eq. 11) then
                     number(1:ndepth, ion) = xnfp_mol(1:ndepth, jmol)

                  else if(mode .eq. 2 .or. mode .eq. 12) then
                     number(1:ndepth, ion) = xn_mol(1:ndepth, jmol)

                  else if(mode .eq. 3 .or. mode .eq. 13) then
                     number(1:ndepth, ion) = xn_mol(1:ndepth, jmol) /
     &                                       xnfp_mol(1:ndepth, jmol)
                  end if

               else   !... DID NOT MATCH CODE
                  id = int(code, in_type)
                  jmol = minloc(abs(int(molcode(1:nummol)) - id), DIM=1)

                  if(int(molcode(jmol)) .eq. id) then
                     number(1:ndepth, ion) = 0.0d0

                  else
                     ion = (code - real(id, re_type)) * 100.0d0 + 1.5d0
     ...
     &                                    abund(id)
               ...
               end if ! SEARCHING FOR MATCH OF code
...
!.... UPDATED TO ATLAS12
!....    MADE vabund(id) -> vabund(id, j) ! 2015 THIS HAS BEEN CHANGED

      use abundances,            only: abund
      use atmosphere_parameters, only: ndepth
      use molecular_vars,        only: molcode, nummol
      use state_vars,            only: rho, xnatom, xne
      use synth_xnmol_vars,      only: xn_mol
      use var_types
...
!--------------------------- molec VARIABLES ---------------------------
      ...
      integer(in_type) :: id
      integer(in_type) :: input_unit = 5
      integer(in_type) :: ion
      integer(in_type) :: j
      integer(in_type) :: jmol
      integer(in_type) :: nn

      logical, save :: mol_read = .false.

      real(re_type) :: c

!--------------------------- molec EXECUTION ---------------------------

      if(.not. mol_read) then
         mol_read = .true.
         read(input_unit, '(i5)') nummol

         do jmol = 1, nummol
            read(input_unit, '(f20.2)') molcode(jmol)
            read(input_unit, '(8e10.3)') xn_mol(1:ndepth, jmol)
            write(6, '(a, f20.2 / (8es10.3))') " xn_mol for code",
     &         molcode(jmol), xn_mol(1:ndepth, jmol)
         end do

         read(input_unit, '(8e10.3)')
         read(input_unit, '(8e10.3)') (xnatom(j), rho(j), j = 1, ndepth)
         read(input_unit, '(8e10.3)')
         read(input_unit, '(8e10.3)') xne(1:ndepth)
      ...
      if(codout .ge. 100.0d0) then ! MOLECULES
         ...
         if(molcode(jmol) .eq. codout) then
            ...
            write(6, '(2a, f10.2)') "ERROR IN MOLEC:",
     &                              " NO DATA FOR MOLECULE", codout
            write(*, '(2a, f10.2)') "ERROR IN MOLEC:",
     &                              " NO DATA FOR MOLECULE", codout
            stop
         end if
...
!.... THIS SUBROUTINE IS UNCHANGED FROM VERSION 108 OF 25 MAY 1995
...
!-------------------------- nelect CONSTANTS ---------------------------
      ...
      integer(in_type), parameter :: nz = 10
      integer(in_type), parameter :: nelemz(nz) = [
     &      1, 2, 6, 11, 12, 13, 14, 19, 20, 26 ]
      integer(in_type), parameter :: nionz(nz) = [
     &      1, 2, 2,  2,  2,  2,  2,  2,  2,  2 ]

      real(re_type), parameter :: factre = 1.0d0 / 0.999d0
      real(re_type), parameter :: factrm = 0.999d0 / 1.001d0
      real(re_type), parameter :: factrp = 1.001d0

!-------------------------- nelect VARIABLES ---------------------------

      character(len=2) :: ee(nz)
...
!-------------------------- nelect EXECUTION ---------------------------
      ...
      if(iter .eq. numit) then
         ee(1:nz) = elem(nelemz(1:nz))
         write(6, '(50x, a // 2x, 20(4x, a2) / )') 
     &      "electron contributions", ee(1:nz), ee(1:nz)
      end if

      xne(1) = p_gas(1) / tk(1) * 0.5d0
         ...
         do
            ...
            do i = 1, nz
               ...
               if(mask(i)) then
                  ...
                  call pfsaha(j, iz, nion, 4, elec(1:ndepth, 1:1))
                  e(i) = elec(j, 1)
                  x(i) = elec(j, 1) * xnatom(j) * abund(iz)
                  xne_new = xne_new + x(i)
               end if
            ...
            xne(j) = xne_new
            xnatom(j) = xn_tot - xne(j)
!!!!        if(xne_error .lt. 5.0d-4) exit ! ORIGINAL LIMIT
            if(xne_error .lt. 1.0d-4) exit ! ATLAS12 LIMIT

            if(j .gt. 1) then
               x1 = 1.0d-5 * xne(j)
               if(xne_error .lt. 0.05d0) x1 = x1 * 10.0d0
               where(x(1:nz) .lt. x1) mask(1:nz) = .false.
            end if
         ...
         if(iter .eq. numit) then
!.... REPLACED 2019 APR
!!!!        forall(i = 1:nz) x(i) = x(i) / xne(j)

            do concurrent(i = 1:nz)
               x(i) = x(i) / xne(j)
            end do

            write(6, '(i3, 20f6.3)') j, x(1:nz), e(1:nz)
         end if
...
!.... 2012 MAR - CHANGED numits TO numit
!.... 2009 JUL - ADDED FUNCTIONS equilh2 AND partfnh2 FROM ATLAS9/12
!.... 2005 AUG - VERSION FROM ATLAS7V_2005JUN22 HASN'T CHANGED
!.... 2002 JULY - UPDATED TO atlas7v.for VERSION OF 08 SEP 2000
!.... 1996 MAR29 - REMOVED EQUIVALNCE OF deq AND frac

!.... MAKE A TABLE OF ALL COMPONENTS OF ALL MOLECULES
...
!          SILICON 3+.......    14.03               14,101,101,101
      ...
      use depart_vars,           only: b_al1, b_c1, b_ca1, b_hyd, b_mg1,
     &                                 b_o1, b_si1, nlteon
...
!.... mode IS A DUMMY HERE, JUST TO BE CONSISTENT WITH ATLAS_ODF
...
!!!!  real(re_type) :: frac(max_d, 6) ! 6 = ATLAS9
      real(re_type) :: frac(max_d, 31) ! 31 = ATLAS7
...
!.... CORRECTIONS TO THE EQUILIBRIUM CONSTANTS FOR NON-LTE
!.... BOB'S USUAL CONVENTION FOR nlteon IS: 0 = LTE, 1 = NLTE
!.... HERE BOB SETS nlteon = -1 AS A SIGNAL TO pfsaha
!.... IN pfsaha IF nlteon = -1 BOB USES b = 1.0, i.e. LTE
!.... SO I WILL USE = .false. INSTEAD

         nlteon = .false.

         call pfsaha(j,  1, 1, 3, frac(1:ndepth, 1:1))
         pf_h = frac(j, 1)
         call pfsaha(j,  6, 1, 3, frac(1:ndepth, 1:1))
         pf_c = frac(j, 1)
         call pfsaha(j,  8, 1, 3, frac(1:ndepth, 1:1))
         pf_o = frac(j, 1)
         call pfsaha(j, 12, 1, 3, frac(1:ndepth, 1:1))
         pf_mg = frac(j, 1)
         call pfsaha(j, 13, 1, 3, frac(1:ndepth, 1:1))
         pf_al = frac(j, 1)
         call pfsaha(j, 14, 1, 3, frac(1:ndepth, 1:1))
         pf_si = frac(j, 1)
         call pfsaha(j, 20, 1, 3, frac(1:ndepth, 1:1))
         pf_ca = frac(j, 1)

!.... HERE BOB SETS nlteon = 0, SO I WILL CONTINUE WITH .false.
         ...
         cpf_h = pf_h / bpf_h  * b_hyd(j, 1)
         cpf_c = pf_c / bpf_c  * b_c1(j, 1)
         cpf_o = pf_o / bpf_o  * b_o1(j, 1)
         cpf_mg = pf_mg / bpf_mg * b_mg1(j, 1)
         cpf_al = pf_al / bpf_al * b_al1(j, 1)
         cpf_si = pf_si / bpf_si * b_si1(j, 1)
         cpf_ca = pf_ca / bpf_ca * b_ca1(j, 1)
...
!.... SETUP equilj FOR ALL nummol
         ...
         do jmol = 1, nummol
            ...
            if(equil(1, jmol) .eq. 0.0d0) then ! ATOMS
               ...
               if(ncomp .gt. 1) then
                  id = molcode(jmol)
                  ion = ncomp - 1
                  call pfsaha(j, id, ncomp, 12, frac(1:ndepth, 1:ncomp))
     ...
     &                           xne(j)**ion
            ...
            else ! FOR EQUIL(1, JMOL) .ne. 0.0D0 = MOLECULES
               ion = nint((molcode(jmol) -
     &                     real(int(molcode(jmol), in_type), re_type)) *
     &                     100.0d0, in_type)
               equilj(jmol) = 0.0d0

!.... 2005 NOV 09 FROM ATLAS9/12
...
!.... UP TO 20000 K
     ...
     &                                (real(ncomp-ion-ion-1, re_type)))
               ...
               if(t(j) .le. 20000.0d0) then ! USE HIGHER LIMIT FOR H2
!!!!           if(t(j) .le. 10000.0d0) then
                  ...
                  do lock = locj1, locj2
                     ki = kcomps(lock)
                     id = id_equa(ki)
                     if(id .eq.  1) equilj(jmol) = equilj(jmol) * cpf_h
                     if(id .eq.  6) equilj(jmol) = equilj(jmol) * cpf_c
                     if(id .eq.  8) equilj(jmol) = equilj(jmol) * cpf_o
                     if(id .eq. 12) equilj(jmol) = equilj(jmol) * cpf_mg
                     if(id .eq. 13) equilj(jmol) = equilj(jmol) * cpf_al
                     if(id .eq. 14) equilj(jmol) = equilj(jmol) * cpf_si
                     if(id .eq. 20) equilj(jmol) = equilj(jmol) * cpf_ca
                  end do

               end if   !.... T(J) .le. 20000.0D0
...
!.... REPLACED 2019 APR
!!!!        forall(ki = 2:n_equa) deq(ki, ki) = 1.0d0
...
!!!! &                  eq(1:n_equa))
            ...
            do ki = 1, n_equa
!!!! FOR TESTING -- IN ATLAS_OS/ODF, THIS TEST IS NOW .gt. 1.0d-4
!!!!           if(abs(eq(ki) / xn(ki)) .gt. 1.0d-3) converged = .false.
               if(abs(eq(ki) / xn(ki)) .gt. 1.0d-4) converged = .false.

               if(eq_old(ki) * eq(ki) .lt. 0.0d0) eq(ki) = eq(ki) *
     &                                                     0.69d0
               xneq = xn(ki) - eq(ki)
               xn100 = xn(ki) * 0.01d0
               ...
               if(xneq .lt. xn100) then
                  xn(ki) = xn(ki) / scale
                  if(eq_old(ki) * eq(ki) .lt. 0.0d0) scale = sqrt(scale)
               else
                  xn100 = xn(ki) * 100.0d0
                  xn(ki) = xneq
               end if
...
!!!!     rho(j) = xnatom(j) * wtmole(j) * 1.660d-24 ! BOB'S VALUE
      ...
      if(iter .eq. numit) then
         write(6, '( / a // t8, a, t19, a, t29, a, t40, a, t50, a,
     &                      t62, a / )')
     &       "in subroutine nmolec: final iteration",
     ...
     &                         MASK=(int(molcode(1:nummol)) .eq. id))
            ...
            if(all(molcode(jmol1:jmol10) .lt. 100000.0d0)) then
               write(6, '(5x, 6f11.2)') molcode(jmol1:jmol10)
            else
               write(6, '(5x, 6i11)') int(molcode(jmol1:jmol10))
            end if

            do j = 1, ndepth
               write(6, '(i5, 6es11.3)') j, (xn_mol(j, jmol),
     &                                       jmol = jmol1, jmol10)
            end do
      ...
      do ki = 2, n_equa
         ...
         if(id .eq. 100) then
!.... REPLACED 2019 APR
!!!!        forall(j = 1:ndepth) xnz(j, ki) = xnz(j, ki) * 0.5d0 /
!!!! &                                        2.4148d15 / t(j) /
!!!! &                                        sqrt(t(j))

            do concurrent(j = 1:ndepth)
               xnz(j, ki) = xnz(j, ki) * 0.5d0 / 2.4148d15 / t(j) /
     &                                   sqrt(t(j))
            end do
            ...
            do j = 1, ndepth
               ...
               if(id .eq.  1) frac(j, 1) = frac(j, 1) / b_hyd(j, 1)
               if(id .eq.  6) frac(j, 1) = frac(j, 1) / b_c1(j, 1)
               if(id .eq.  8) frac(j, 1) = frac(j, 1) / b_o1(j, 1)
               if(id .eq. 12) frac(j, 1) = frac(j, 1) / b_mg1(j, 1)
               if(id .eq. 13) frac(j, 1) = frac(j, 1) / b_al1(j, 1)
               if(id .eq. 14) frac(j, 1) = frac(j, 1) / b_si1(j, 1)
               if(id .eq. 20) frac(j, 1) = frac(j, 1) / b_ca1(j, 1)
      ...
      do jmol = 1, nummol
         ...
         if(equil(1, jmol) .eq. 0.0d0) then
            ...
            do j = 1, ndepth
               ...
               xnfp_mol(j, jmol) = xn_mol(j, jmol) / frac(j, 1)
            end do

         else   ! EQUIL(1, JMOL) .ne. 0.0D0

            xnfp_mol(1:ndepth, jmol) = exp(equil(1, jmol) /
     &                                     tkev(1:ndepth))
            amass = 0.0d0
            locj1 = locj(jmol)
            locj2 = locj(jmol + 1) - 1
            ...
            do lock = locj1, locj2
               ...
               if(ki .eq. n_equa + 1) then
!.... REPLACED 2019 APR
!!!!              forall(j = 1:ndepth) xnfp_mol(j, jmol) =
!!!! &                                 xnfp_mol(j, jmol) /
!!!! &                                 xnz(j, n_equa)

                  do concurrent(j = 1:ndepth)
                     xnfp_mol(j, jmol) = xnfp_mol(j, jmol) /
     &                                   xnz(j, n_equa)
                  end do
...
!.... REPLACED 2019 APR
!!!!              forall(j = 1:ndepth) xnfp_mol(j, jmol) =
!!!! &                                 xnfp_mol(j, jmol) * xnz(j, ki)

                  do concurrent(j = 1:ndepth)
                     xnfp_mol(j, jmol) = xnfp_mol(j, jmol) * xnz(j, ki)
                  end do
...
!.... REPLACED 2019 APR
!!!!        forall(j = 1:ndepth) xnfp_mol(j, jmol) = xnfp_mol(j, jmol) *
!!!! &         1.8786d20 * sqrt((amass * t(j))**3)

            do concurrent(j = 1:ndepth)
               xnfp_mol(j, jmol) = xnfp_mol(j, jmol) * 1.8786d20 *
     &                             sqrt((amass * t(j))**3)
            end do
      ...
      if(iter .eq. numit) then
         write(6, '(/ t21, a / i5, a)')
     &      "number densities / partition functions", nummol,
     &      " atoms and molecules"

         do jmol = 1, nummol
            write(6, '(/ a, f13.2 / (8es10.3))') " atom/ion/molecule =",
     &         molcode(jmol), xnfp_mol(1:ndepth, jmol)
         end do
...
!.... SPECIFIC ELEMENTS AND IONS WITH DIFFERENT VALUES
      ...
      else if(iz .eq. 5 .and. ion .eq. 1) then   !.... 5.01    FROM ATLAS12
...
!!!!     pfg = 2.0d0
      ...
      else if(iz .eq. 5 .and. ion .eq. 2) then   !.... 5.02    FROM ATLAS12
...
!!!!     pfg = 2.0d0
      ...
      else if(iz .eq. 5 .and. ion .eq. 4) then   !.... 5.04    FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 10192.63d0)   ! FROM ATLAS12
...
!!!!     pfg = 2.0d0
      ...
      else if(iz .eq. 6 .and. ion .eq. 3) then   !.... 6.03    FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 15316.2d0)    ! FROM ATLAS12
...
!!!!     pfg = 2.0d0
      ...
      else if(iz .eq. 7 .and. ion .eq. 4) then   !.... 7.04    FROM ATLAS12
...
!!!!     pfg = 4.0d0
      ...
      else if(iz .eq. 8 .and. ion .eq. 1) then   !.... 8.01    FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 20273.27d0) + ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 43185.74d0) + ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 60324.79d0)   ! FROM ATLAS12

      else if(iz .eq. 8 .and. ion .eq. 3) then   !.... 8.03
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 385.9d0) +
     &                 2.0d0 * exp(-hckt * 71439.8d0) +
     &                 4.0d0 * exp(-hckt * 71570.1d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 71755.5d0)    ! FROM ATLAS12

!!!!  else if(iz .eq. 8 .and. ion .eq. 5) then   !.... 8.05
!!!!     pfg = 2.0d0

      else if(iz .eq. 8 .and. ion .eq. 5) then   !.... 8.05    FROM ATLAS12
         pfg = 2.0d0 + 2.0d0 * exp(-hckt * 96375.0d0) +
     &                 4.0d0 * exp(-hckt * 96907.5d0)

      else if(iz .eq. 9 .and. ion .eq. 0) then   !.... 9.00
         pfg = 4.0d0 + 2.0d0 * exp(-hck/t * 404.1d0)

      else if(iz .eq. 9 .and. ion .eq. 1) then   !.... 9.01
         pfg = 5.0d0 + 3.0d0 * exp(-hckt * 341.0d0) +
     &                 1.0d0 * exp(-hckt * 489.9d0) +
     &                 5.0d0 * exp(-hckt * 20873.4d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 44918.1d0)    ! FROM ATLAS12

!!!!  else if(iz .eq. 9 .and. ion .eq. 2) then   !.... 9.02
!!!!     pfg = 4.0d0

      else if(iz .eq. 9 .and. ion .eq. 2) then   !.... 9.02  ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 25238.2d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 53541.2d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 74194.7d0)    ! FROM ATLAS12

      else if(iz .eq. 9 .and. ion .eq. 4) then   !.... 9.04
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 744.5d0) +
     &                 2.0d0 * exp(-hckt * 85790.2d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 86043.5d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 86407.0d0)    ! FROM ATLAS12

      else if(iz .eq. 9 .and. ion .eq. 5) then   !.... 9.05    FROM ATLAS12
     ...
     &                 4.0d0 * exp(-hckt * 96907.5d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 25840.8d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 55750.6d0)    ! FROM ATLAS12

      else if(iz .eq. 10 .and. ion .eq. 3) then  !.... 10.03
         pfg = 4.0d0 + 6.0d0 * exp(-hckt * 41234.6d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 41279.5d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 62434.6d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 62441.3d0)    ! FROM ATLAS12

      else if(iz .eq. 10 .and. ion .eq. 4) then  !.... 10.04
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 414.0d0) +
     &                 5.0d0 * exp(-hckt * 1112.0d0) +
     &                 5.0d0 * exp(-hckt * 30291.5d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 63913.6d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 88360.0d0)    ! FROM ATLAS12

      else if(iz .eq. 10 .and. ion .eq. 5) then  !.... 10.05
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 1310.0d0) +
     &                 2.0d0 * exp(-hckt * 100261.0d0) + ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 100704.0d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 101347.0d0)   ! FROM ATLAS12
         ...
         pfg = 5.0d0 + 3.0d0 * exp(-hckt * 642.9d0) +
     &                 1.0d0 * exp(-hckt * 920.4d0) +
     &                 5.0d0 * exp(-hckt * 30839.8d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 66496.0d0)    ! FROM ATLAS12

      else if(iz .eq. 11 .and. ion .eq. 4) then  !.... 11.04
         pfg = 4.0d0 + 6.0d0 * exp(-hckt * 48330.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 48366.0d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 73218.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 73255.0d0)    ! FROM ATLAS12

      else if(iz .eq. 11 .and. ion .eq. 5) then  !.... 11.05
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 414.0d0) +
     &                 5.0d0 * exp(-hckt * 1112.0d0) +
     &                 5.0d0 * exp(-hckt * 35498.0d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 74414.0d0)    ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 35926.0d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 77279.0d0)    ! FROM ATLAS12

      else if(iz .eq. 12 .and. ion .eq. 5) then  !.... 12.05
         pfg = 4.0d0 + 6.0d0 * exp(-hckt * 55356.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 55372.8d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 83920.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 84028.4d0)    ! FROM ATLAS12
     ...
     &                         exp(-hckt * 3829.0d0) +
     &                 5.0d0 * exp(-hckt * 41167.0d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 88213.0d0)    ! FROM ATLAS12

      else if(iz .eq. 14 .and. ion .eq. 0) then  !.... 14.00
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 77.115d0) +
     &                 5.0d0 * exp(-hckt * 223.157d0) +
     &                 5.0d0 * exp(-hckt * 6298.850d0)   ! FROM ATLAS12

      else if(iz .eq. 14 .and. ion .eq. 1) then  !.... 14.01
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 287.32d0) +
     &                 2.0d0 * exp(-hckt * 42824.35d0) + ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 42932.68d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 43107.97d0)   ! FROM ATLAS12

      else if(iz .eq. 14 .and. ion .eq. 2) then  !.... 14.02   FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 8882.31d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 21575.63d0)   ! FROM ATLAS12

      else if(iz .eq. 15 .and. ion .eq. 2) then  !.... 15.02
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 559.14d0) +
     &                 2.0d0 * exp(-hckt * 56021.67d0) + ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 57125.98d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 57454.00d0) + ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 74916.85d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 74945.86d0)   ! FROM ATLAS12

      else if(iz .eq. 15 .and. ion .eq. 3) then  !.... 15.03 ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 68615.17d0)
         ...
         pfg = 2.0d0 + 2.0d0 * exp(-hckt * 88651.87d0) + ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 89447.25d0)   ! FROM ATLAS12

      else if(iz .eq. 16 .and. ion .eq. 0) then  !.... 16.00
         pfg = 5.0d0 + 3.0d0 * exp(-hckt * 396.055d0) +
     &                 1.0d0 * exp(-hckt * 573.640d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 573.640d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 9238.609d0)   ! FROM ATLAS12

      else if(iz .eq. 16 .and. ion .eq. 1) then  !.... 16.01
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 14852.94d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 14884.73d0) + ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 24524.83d0) + ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 24571.54d0)   ! FROM ATLAS12

      else if(iz .eq. 16 .and. ion .eq. 2) then  !.... 16.02
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 298.69d0) +
     &                 5.0d0 * exp(-hckt * 833.08d0) +
     &                 5.0d0 * exp(-hckt * 11322.7d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 27161.0d0)    ! FROM ATLAS12

      else if(iz .eq. 16 .and. ion .eq. 3) then  !.... 16.03
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 951.43d0) +
     &                 2.0d0 * exp(-hckt * 71184.1d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 71528.7d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 72074.4d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 94103.1d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 94150.4d0)    ! FROM ATLAS12

      else if(iz .eq. 16 .and. ion .eq. 4) then  !.... 16.04   FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 11653.58d0) + ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 27878.02d0)   ! FROM ATLAS12

      else if(iz .eq. 17 .and. ion .eq. 2) then  !.... 17.02
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 18053.0d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 18118.6d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 29812.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 29907.0d0)    ! FROM ATLAS12

      else if(iz .eq. 17 .and. ion .eq. 3) then  !.... 17.03
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 491.0d0) +
     &                 5.0d0 * exp(-hckt * 1341.0d0) +
     &                 5.0d0 * exp(-hckt * 13767.6d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 32547.8d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 65000.0d0)    ! FROM ATLAS12

      else if(iz .eq. 17 .and. ion .eq. 4) then  !.... 17.04
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 1492.0d0) +
     &                 2.0d0 * exp(-hckt * 86000.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 86538.0d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 87381.0d0)    ! FROM ATLAS12

      else if(iz .eq. 17 .and. ion .eq. 5) then  !.... 17.05 ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 14010.004d0) +! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 33265.724d0)  ! FROM ATLAS12

      else if(iz .eq. 18 .and. ion .eq. 3) then  !.... 18.03
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 21090.4d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 21219.3d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 34855.5d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 35032.6d0)    ! FROM ATLAS12

      else if(iz .eq. 18 .and. ion .eq. 4) then  !.... 18.04
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 765.0d0) +
     &                 5.0d0 * exp(-hckt * 2030.0d0) +
     &                 5.0d0 * exp(-hckt * 16298.9d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 37912.0d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 84100.0d0)    ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 16384.1d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 38546.3d0)    ! FROM ATLAS12

      else if(iz .eq. 19 .and. ion .eq. 4) then  !.... 19.04
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 24012.5d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 24249.6d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 39758.1d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 40080.2d0)    ! FROM ATLAS12

      else if(iz .eq. 19 .and. ion .eq. 5) then  !.... 19.05
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 1132.0d0) +
     &                 5.0d0 * exp(-hckt * 2924.0d0) +
     &                 5.0d0 * exp(-hckt * 18977.8d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 43358.8d0)    ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 7125.299d0)   ! FROM ATLAS12
...
!!!! &                 5.0d0 * exp(-hckt * 8576.149d0) +  Bob typo
     &                 5.0d0 * exp(-hckt * 9576.149d0) + ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 22446.202d0)  ! FROM ATLAS12

      else if(iz .eq. 34 .and. ion .eq. 1) then  !.... 34.01
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 13168.2d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 13784.4d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 23038.3d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 23894.8d0)    ! FROM ATLAS12

      else if(iz .eq. 34 .and. ion .eq. 2) then  !.... 34.02
         pfg = 1.0d0 + 3.0d0 * exp(-hckt * 1741.0d0) +
     &                 5.0d0 * exp(-hckt * 3937.0d0) +
     &                 5.0d0 * exp(-hckt * 13032.0d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 28430.0d0)    ! FROM ATLAS12
     ...
     &                 5.0d0 * exp(-hckt * 12089.1d0)    ! FROM ATLAS12

      else if(iz .eq. 35 .and. ion .eq. 2) then  !.... 35.02
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 15042.0d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 16301.0d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 26915.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 28579.0d0)    ! FROM ATLAS12
         ...
         pfg = 5.0d0 + 3.0d0 * exp(-hckt * 3136.4d0) +
     &                 1.0d0 * exp(-hckt * 3837.5d0) +
     &                 5.0d0 * exp(-hckt * 14644.3d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 33079.6d0)    ! FROM ATLAS12
         ...
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 14555.50d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 14836.24d0)   ! FROM ATLAS12

      else if(iz .eq. 39 .and. ion .eq. 0) then  !.... 39.00
         pfg = 4.0d0 + 6.0d0 * exp(-hckt * 530.36d0)

      else if(iz .eq. 39 .and. ion .eq. 1) then  !.... 39.01 ! FROM ATLAS12
...
!!!!     pfg = 2.0d0
      ...
      else if(iz .eq. 39 .and. ion .eq. 2) then  !.... 39.02  FROM ATLAS12
...
!!!!    CITING EPSTEIN AND READER, JOSA, 65, 310-314, 1975
     ...
     &                 1.0d0 * exp(-hckt * 4196.85d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 4376.28d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 4186.11d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 4870.53d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 5023.41d0) +  ! FROM ATLAS12
     &                 7.0d0 * exp(-hckt * 5249.07d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 5540.54d0) +  ! FROM ATLAS12
     &                11.0d0 * exp(-hckt * 5888.93d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 5101.68d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 8057.30d0)    ! FROM ATLAS12

      else if(iz .eq. 40 .and. ion .eq. 1) then  !.... 40.01
         pfg = 4.0d0 +  6.0d0 * exp(-hckt * 314.67d0) +
     &                  8.0d0 * exp(-hckt * 763.44d0) +
     &                 10.0d0 * exp(-hckt * 1322.91d0) + ! FROM ATLAS12
     ...
     &   4.0d0*exp(-hckt * 9742.80d0) + 6.0d0*exp(-hckt * 9968.75d0)
         ...
         pfg = 5.0d0 + 7.0d0 * exp(-hckt * 680.5d0) +
     &                 9.0d0 * exp(-hckt * 1485.7d0) +
     &                 5.0d0 * exp(-hckt * 5742.8d0) +   ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 8062.7d0) +   ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 8327.0d0) +   ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 8839.7d0) +   ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 11049.9d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 23974.9d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 18400.8d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 18804.7d0) +  ! FROM ATLAS12
     &                 7.0d0 * exp(-hckt * 19535.3d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 25066.9d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 36473.7d0)    ! FROM ATLAS12
     ...
     &                 4.0d0 * exp(-hckt * 1142.79d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 1586.90d0) +  ! FROM ATLAS12
     &                 8.0d0 * exp(-hckt * 2154.11d0) +  ! FROM ATLAS12
     &                10.0d0 * exp(-hckt * 2805.36d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 4998.17d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 5297.92d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 5965.45d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 8410.90d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 8705.32d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 9043.14d0) +  ! FROM ATLAS12
     &                 8.0d0 * exp(-hckt * 9497.52d0) +  ! FROM ATLAS12
     &                 8.0d0 * exp(-hckt * 8827.00d0) +  ! FROM ATLAS12
     &                10.0d0 * exp(-hckt * 9328.88d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 9439.08d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 10237.51d0)   ! FROM ATLAS12
     ...
     &                 3.0d0 * exp(-hckt * 2356.76d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 2629.07d0) +  ! FROM ATLAS12
     &                 7.0d0 * exp(-hckt * 3029.57d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 3542.50d0) +  ! FROM ATLAS12
     &                11.0d0 * exp(-hckt * 4146.00d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 5562.26d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 6192.33d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 7261.33d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 7505.78d0) +  ! FROM ATLAS12
     &                 7.0d0 * exp(-hckt * 7900.65d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 8320.40d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 9509.67d0) +  ! FROM ATLAS12
     &                11.0d0 * exp(-hckt * 9812.56d0) +  ! FROM ATLAS12
     &                13.0d0 * exp(-hckt * 10186.41d0)   ! FROM ATLAS12

      else if(iz .eq. 41 .and. ion .eq. 2) then  !.... 41.02
         pfg = 4.0d0 +  6.0d0 * exp(-hckt * 515.8d0) +
     &                  8.0d0 * exp(-hckt * 1176.6d0) +
     &                 10.0d0 * exp(-hckt * 1939.0d0) +
     &                 2.0d0 * exp(-hckt * 8664.3d0) +   ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 8607.5d0) +   ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 9593.7d0) +   ! FROM ATLAS12
     &                 8.0d0 * exp(-hckt * 9236.1d0) +   ! FROM ATLAS12
     &                10.0d0 * exp(-hckt * 9804.5d0) +   ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 10912.2d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 13094.0d0) +  ! FROM ATLAS12
     &                10.0d0 * exp(-hckt * 12916.0d0) +  ! FROM ATLAS12
     &                12.0d0 * exp(-hckt * 13263.8d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 19975.0d0) +  ! FROM ATLAS12
     &                 8.0d0 * exp(-hckt * 19861.0d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 25220.2d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 25735.2d0) +  ! FROM ATLAS12
     &                 8.0d0 * exp(-hckt * 26463.7d0) +  ! FROM ATLAS12
     &                10.0d0 * exp(-hckt * 27373.5d0)    ! FROM ATLAS12
...
!!!!  7JUN04 CORRECTED BY JOHN LAIRD  SECOND TERM IS LOW
         pfg = 6.0d0 + 10.0d0 * exp(-hckt * 2572.89d0) + ! FROM ATLAS12
     &                  8.0d0 * exp(-hckt * 3250.91d0) + ! FROM ATLAS12
     &                  6.0d0 * exp(-hckt * 3700.54d0) + ! FROM ATLAS12
     &                  4.0d0 * exp(-hckt * 4002.57d0) + ! FROM ATLAS12
     &                  2.0d0 * exp(-hckt * 4178.75d0)   ! FROM ATLAS12

      else if(iz .eq. 43 .and. ion .eq. 1) then  !.... 43.01
!!!!  7JUN04 CORRECTED BY JOHN LAIRD  SECOND TERM IS LOW
         pfg = 7.0d0 + 9.0d0 * exp(-hckt * 3461.27d0) +  ! FROM ATLAS12
     &                 7.0d0 * exp(-hckt * 4217.17d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 4669.22d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 4961.14d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 5100.98d0)    ! FROM ATLAS12
     ...
     &                  9.0d0 * exp(-hckt * 6545.03d0) + ! FROM ATLAS12
     &                  7.0d0 * exp(-hckt * 8084.12d0) + ! FROM ATLAS12
     &                  5.0d0 * exp(-hckt * 9183.66d0) + ! FROM ATLAS12
     &                  9.0d0 * exp(-hckt * 7483.07d0) + ! FROM ATLAS12
     &                  7.0d0 * exp(-hckt * 8575.42d0) + ! FROM ATLAS12
     &                  5.0d0 * exp(-hckt * 9057.64d0) + ! FROM ATLAS12
     &                  3.0d0 * exp(-hckt * 9072.98d0) + ! FROM ATLAS12
     &                  1.0d0 * exp(-hckt * 8492.37d0) + ! FROM ATLAS12
     &                  7.0d0 * exp(-hckt * 8770.93d0) + ! FROM ATLAS12
     &                  5.0d0 * exp(-hckt * 8043.69d0) + ! FROM ATLAS12
     &                  3.0d0 * exp(-hckt * 9620.29d0) + ! FROM ATLAS12
     &                  9.0d0 * exp(-hckt * 9120.63d0)   ! FROM ATLAS12

      else if(iz .eq. 44 .and. ion .eq. 1) then  !.... 44.01
         pfg = 10.0d0 + 8.0d0 * exp(-hckt * 1523.1d0) +
     &                  6.0d0 * exp(-hckt * 2493.9d0) +
     &                  4.0d0 * exp(-hckt * 3104.2d0) +
!!!! &                  4.0d0 * exp(hckt * 3104.2d0) +   ! BOB'S TYPO
!!!! IN THE ORIGINAL THERE IS A MISSING RETURN HERE, PRODUCING AN ERROR
     &                   6.0d0 * exp(-hckt * 8256.7d0) + ! FROM ATLAS12
     &                   4.0d0 * exp(-hckt * 8477.4d0) + ! FROM ATLAS12
     &                   2.0d0 * exp(-hckt * 9373.4d0) + ! FROM ATLAS12
     &                  10.0d0 * exp(-hckt * 9151.6d0)   ! FROM ATLAS12
...
!!!! IN THE ORIGINAL THERE IS A MISSING RETURN HERE, PRODUCING AN ERROR
     &                 7.0d0 * exp(-hckt * 27162.8d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 41111.7d0)    ! FROM ATLAS12
     ...
     &                  6.0d0 * exp(-hckt * 3309.86d0) + ! FROM ATLAS12
     &                  4.0d0 * exp(-hckt * 5657.97d0) + ! FROM ATLAS12
     &                  8.0d0 * exp(-hckt * 5690.97d0) + ! FROM ATLAS12
     &                  6.0d0 * exp(-hckt * 7791.23d0) + ! FROM ATLAS12
     &                  6.0d0 * exp(-hckt * 9221.22d0)   ! FROM ATLAS12

      else if(iz .eq. 45 .and. ion .eq. 1) then  !.... 45.01
         pfg = 9.0d0 + 7.0d0 * exp(-hckt * 2401.3d0) +
     &                 5.0d0 * exp(-hckt * 3580.7d0) +
     &                 5.0d0 * exp(-hckt * 8164.4d0) +   ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 10760.8d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 10515.0d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 11643.7d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 14855.4d0) +  ! FROM ATLAS12
     &                11.0d0 * exp(-hckt * 16884.8d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 18540.4d0) +  ! FROM ATLAS12
     &                 7.0d0 * exp(-hckt * 19792.4d0)    ! FROM ATLAS12

      else if(iz .eq. 45 .and. ion .eq. 2) then  !.... 45.02
         pfg = 10.0d0 + 8.0d0 * exp(-hckt * 2147.8d0) +
     &                  6.0d0 * exp(-hckt * 3485.7d0) +
     &                  4.0d0 * exp(-hckt * 4322.0d0) +
     &                  6.0d0 * exp(-hckt * 11062.3d0) + ! FROM ATLAS12
     &                  4.0d0 * exp(-hckt * 10997.1d0) + ! FROM ATLAS12
     &                  2.0d0 * exp(-hckt * 12469.8d0) + ! FROM ATLAS12
     &                 10.0d0 * exp(-hckt * 14044.0d0) + ! FROM ATLAS12
     &                  8.0d0 * exp(-hckt * 15256.8d0) + ! FROM ATLAS12
     &                  4.0d0 * exp(-hckt * 16870.7d0) + ! FROM ATLAS12
     &                  2.0d0 * exp(-hckt * 18303.7d0) + ! FROM ATLAS12
     &                 12.0d0 * exp(-hckt * 19490.2d0) + ! FROM ATLAS12
     &                  6.0d0 * exp(-hckt * 19528.5d0)   ! FROM ATLAS12

      else if(iz .eq. 46 .and. ion .eq. 0) then  !.... 46.00 ! FROM ATLAS12
         pfg = 1.0d0 + 7.0d0 * exp(-hckt * 6564.11d0) +
     &                 5.0d0 * exp(-hckt * 7754.99d0)

      else if(iz .eq. 46 .and. ion .eq. 1) then  !.... 46.01
         pfg = 6.0d0 + 4.0d0 * exp(-hckt * 3539.0d0)

      else if(iz .eq. 46 .and. ion .eq. 2) then  !.... 46.02
         pfg = 9.0d0 + 7.0d0 * exp(-hckt * 3229.3d0) +
     &                 5.0d0 * exp(-hckt * 4687.5d0) +
     &                 5.0d0 * exp(-hckt * 10229.3d0) +  ! FROM ATLAS12
     &                 3.0d0 * exp(-hckt * 13468.9d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 13697.5d0) +  ! FROM ATLAS12
     &                 5.0d0 * exp(-hckt * 14634.4d0) +  ! FROM ATLAS12
     &                 9.0d0 * exp(-hckt * 17879.3d0)    ! FROM ATLAS12
...
!!!!  else if(iz .eq. 51 .and. ion .eq. 3) then  !.... 51.03
     ...
     &                 5.0d0 * exp(-hckt * 13727.2d0) +  ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 29501.3d0)    ! FROM ATLAS12

      else if(iz .eq. 53 .and. ion .eq. 2) then  !.... 53.02
         pfg = 4.0d0 + 4.0d0 * exp(-hckt * 11711.2d0) +  ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 14901.9d0) +  ! FROM ATLAS12
     &                 2.0d0 * exp(-hckt * 24299.3d0) +  ! FROM ATLAS12
     &                 4.0d0 * exp(-hckt * 29636.8d0)    ! FROM ATLAS12

      else if(iz .eq. 54 .and. ion .eq. 1) then  !.... 54.01
         pfg = 4.0d0 + 2.0d0 * exp(-hckt * 10537.01d0)

      else if(iz .eq. 54 .and. ion .eq. 2) then  !.... 54.02
         pfg = 5.0d0 + 3.0d0 * exp(-hckt * 9794.6d0) +
     &                 1.0d0 * exp(-hckt * 8131.0d0) +
     &                 5.0d0 * exp(-hckt * 17098.73d0) + ! FROM ATLAS12
     &                 1.0d0 * exp(-hckt * 36102.94d0)   ! FROM ATLAS12
         ...
         pfg = 2.0d0 + 4.0d0 * exp(-hckt * 4873.852d0) + ! FROM ATLAS12
     &                 6.0d0 * exp(-hckt * 5674.807d0)   ! FROM ATLAS12
...
!.... THIS VERSION FROM atlas_odf_pops.f
...
!.... 2002 JUL - UPDATED TO atlas7v.for VERSION OF 08 SEP 2000
!.... 1999 NOV - UPDATE TO BOB'S VERSION IN atlas9 OF 14 FEB 1999
!.... 1993 JAN - TO USE THE SAME CUTOFF AS BOB TO GET THE SAME RESULT
!.... 1988 MAR - TO AVOID OVER/UNDERFLOW AT LOW DENSITY


      use depart_vars,        only: b_al1, b_b1, b_c1, b_c2, b_ca1,
     &                              b_ca2, b_he1, b_he2, b_hyd, b_k1,
     &                              b_mg1, b_mg2, b_na1, b_o1, b_si1,
     &                              b_si2, nlteon
      use physical_constants, only: e_esu, hydip, pi4, tenlog, waveno_ev
      use state_vars,         only: chargesq, p_gas, xne
      use synth_ionpots           ! lower_lim, potion
...
!.... ENERGY LEVELS, IN CM**(-1)
      ...
      real(re_type), parameter :: e_al1(9) = [
     &        74.707d0,  25347.756d0,  29097.11d0,   32436.241d0,
     &     32960.363d0,  37689.413d0,  38932.139d0,  40275.903d0,
     &     41319.377d0 ]

      real(re_type), parameter :: e_b1(7) = [
     &        10.17d0,   28810.0d0,    40039.65d0,   47856.99d0, 
     &     48613.01d0,   54767.74d0,   55010.08d0 ]
     
      real(re_type), parameter :: e_c1(14) = [
     &        29.60d0,   10192.66d0,   21648.02d0,   33735.20d0,  
     &      60373.00d0,  61981.82d0,   64088.85d0,   68856.33d0,   
     &      69722.00d0,  70743.95d0,   71374.90d0,   72610.72d0,  
     &      73975.91d0,  75254.93d0 ]

      real(re_type), parameter :: e_c2(6) = [
     &        42.48d0,   43035.8d0,    74931.11d0,   96493.74d0, 
     &    110652.10d0,  116537.65d0 ]

      real(re_type), parameter :: e_ca1(8) = [
     &         0.0d0,    15263.089d0,  20356.265d0,  21849.634d0,  
     &     23652.304d0,  31539.495d0,  33317.264d0,  35831.203d0 ]

      real(re_type), parameter :: e_ca2(5) = [
     &         0.0d0,    13686.60d0,   25340.10d0,   52166.93d0,   
     &     56850.78d0 ]

      real(re_type), parameter :: e_he1(29) = [
     &         0.0d0,   159856.069d0, 166277.546d0, 169087.007d0, 
     &    171135.000d0, 183236.892d0, 184864.936d0, 185564.694d0, 
     &    186101.654d0, 186105.065d0, 186209.471d0, 190298.210d0, 
     &    190940.331d0, 191217.14d0,  191444.588d0, 191446.559d0, 
     &    191451.80d0,  191452.08d0,  191492.817d0, 193347.089d0, 
     &    193663.627d0, 193800.78d0,  193917.245d0, 193918.391d0,  
     &    193921.31d0,  193921.37d0,  193922.5d0,   193922.5d0,   
     &    193942.57d0 ]

      real(re_type), parameter :: e_he2(6) = [
     &         0.0d0,   329182.321d0, 390142.359d0, 411477.925d0, 
     &    421353.135d0, 426717.413d0 ]

      real(re_type), parameter :: e_hyd(6) = [
     &         0.0d0,    82259.105d0,  97492.302d0, 102823.893d0, 
     &    105291.651d0, 106632.160d0 ]

      real(re_type), parameter :: e_k1(8) = [
     &         0.0d0,    12985.170d0,  13042.876d0,  21026.551d0, 
     &     21534.680d0,  21536.988d0,  24701.382d0,  24720.139d0 ]

      real(re_type), parameter :: e_mg1(11) = [
     &         0.0d0,    21890.854d0,  35051.264d0,  41197.403d0, 
     &     43503.333d0,  46403.065d0,  47847.797d0,  47957.034d0,  
     &     49346.729d0,  51872.526d0,  52556.206d0 ]

      real(re_type), parameter :: e_mg2(6) = [
     &         0.0d0,    35730.36d0,   69804.95d0,   71490.54d0,  
     &     80639.85d0,   92790.51d0 ]

      real(re_type), parameter :: e_na1(8) = [
     &         0.0d0,    16956.172d0,  16973.368d0,  25739.991d0, 
     &     29172.889d0,  29172.839d0,  30266.99d0,   30272.58d0 ]

      real(re_type), parameter :: e_o1(13) = [
     &        77.975d0,  15867.862d0,  33792.583d0,  73768.200d0,  
     &     76794.978d0,  86629.089d0,  88630.977d0,  95476.728d0, 
     &     96225.049d0,  97420.748d0,  97488.476d0,  99094.065d0,  
     &     99681.051d0 ]

      real(re_type), parameter :: e_si1(11) = [
     &       149.681d0,   6298.850d0,  15394.370d0,  33326.053d0,  
     &     39859.920d0,  40991.884d0,  45303.310d0,  47284.061d0,  
     &     47351.554d0,  48161.459d0,  49128.131d0 ]

      real(re_type), parameter :: e_si2(6) = [
     &       191.55d0,   43002.27d0,   55319.11d0,   65500.73d0,  
     &     76665.61d0,   79348.67d0 ]

!.... STATISTICAL WEIGHTS FOR THE ENERGY LEVELS GIVEN ABOVE

      real(re_type), parameter :: g_al1(9) = [
     &      6.0d0,  2.0d0, 12.0d0, 10.0d0,  6.0d0,  
     &      2.0d0, 10.0d0,  6.0d0, 14.0d0 ]

      real(re_type), parameter :: g_b1(7) = [
     &      6.0d0, 12.0d0,  2.0d0, 10.0d0,  6.0d0, 
     &     10.0d0,  2.0d0 ]

      real(re_type), parameter :: g_c1(14) = [
     &      9.0d0,  5.0d0,  1.0d0,  5.0d0,  9.0d0,  
     &      3.0d0, 15.0d0,  3.0d0, 15.0d0,  3.0d0,
     &      9.0d0,  5.0d0,  1.0d0,  9.0d0 ]

      real(re_type), parameter :: g_c2(6) = [
     &      6.0d0, 12.0d0, 10.0d0,  2.0d0,  6.0d0,  2.0d0 ]

      real(re_type), parameter :: g_ca1(8) = [
     &      1.0d0,  9.0d0, 15.0d0,  5.0d0,  3.0d0,  
     &      3.0d0,  1.0d0, 21.0d0 ]

      real(re_type), parameter :: g_ca2(5) = [
     &      2.0d0, 10.0d0,  6.0d0,  2.0d0, 10.0d0 ]

      real(re_type), parameter :: g_he1(29) = [
     &      1.0d0,  3.0d0,  1.0d0,  9.0d0,  3.0d0,  
     &      3.0d0,  1.0d0,  9.0d0, 15.0d0,  5.0d0,
     &      3.0d0,  3.0d0,  1.0d0,  9.0d0, 15.0d0,  
     &      5.0d0, 21.0d0,  7.0d0,  3.0d0,  3.0d0, 
     &      1.0d0,  9.0d0, 15.0d0,  5.0d0, 21.0d0, 
     &      7.0d0, 27.0d0,  9.0d0,  3.0d0 ]

      real(re_type), parameter :: g_he2(6) = [
     &      2.0d0,  8.0d0, 18.0d0, 32.0d0, 50.0d0, 72.0d0 ]

      real(re_type), parameter :: g_hyd(6) = [
     &      2.0d0,  8.0d0, 18.0d0, 32.0d0, 50.0d0, 72.0d0 ]

      real(re_type), parameter :: g_k1(8) = [
     &      2.0d0,  2.0d0,  4.0d0,  2.0d0,  6.0d0,  
     &      4.0d0,  2.0d0,  4.0d0 ]

      real(re_type), parameter :: g_mg1(11) = [
     &      1.0d0,  9.0d0,  3.0d0,  3.0d0,  1.0d0,  
     &      5.0d0,  9.0d0, 15.0d0,  3.0d0,  3.0d0, 
     &      1.0d0 ]

      real(re_type), parameter :: g_mg2(6) = [
     &      2.0d0,  6.0d0,  2.0d0, 10.0d0,  6.0d0,  2.0d0 ]

      real(re_type), parameter :: g_na1(8) = [
     &      2.0d0,  2.0d0,  4.0d0,  2.0d0,  6.0d0,  
     &      4.0d0,  2.0d0,  4.0d0 ]

      real(re_type), parameter :: g_o1(13) = [
     &      9.0d0,  5.0d0,  1.0d0,  5.0d0,  3.0d0, 
     &     15.0d0,  9.0d0,  5.0d0,  3.0d0, 25.0d0, 
     &     15.0d0, 15.0d0,  9.0d0 ]

      real(re_type), parameter :: g_si1(11) = [
     &      9.0d0,  5.0d0,  1.0d0,  5.0d0,  9.0d0,  
     &      3.0d0, 15.0d0,  3.0d0,  5.0d0, 15.0d0, 
     &      9.0d0 ]

      real(re_type), parameter :: g_si2(6) = [
     &      6.0d0, 12.0d0, 10.0d0,  2.0d0,  2.0d0, 10.0d0 ]

      real(re_type), parameter :: scale(4) = [
...
!-------------------------- pfsaha VARIABLES ---------------------------
      ...
      integer(in_type), save :: nnn(6, 374)
...
!!!! OLD VALUES - SWITCH BACK TO TEST
...
!!!! & 510064491,  82710872, 142718412, 232328712, 348341572,   690000,!AEL89.00
     ...
     & 200020001, 200020001, 200020001, 200020001, 200020001, 66702900/!G   7.06

      data nnn(1:6, 367:374) /
     & 874789691, 924795711,  99410492, 115213492, 169022242,  1361307,!D+F 8.00
     & 424151091, 622874781,  91312832, 221842502,  79914013,  3510711,!D+F 8.01
     &  95610702, 118113032, 149619922, 329761642, 101914173,  5488500,!D+F 8.02
     & 603567171, 775391141, 106612482, 143716252, 181420032,  7739300,!D+F 8.03
     & 124420321, 306943181, 606281181, 101712232, 142916342, 11387300,!D+F 8.04
     & 215026541, 323137551, 421546491, 508255151, 594863811, 13807900,!AEL 8.05
     & 100010001, 100010001, 100010001, 100010001, 100010001, 73927400,!G   8.06
     & 200020001, 200020001, 200020001, 200020001, 200020001, 87138700/!G   8.07
!.... AT THIS POINT IN THE ORIGINAL THERE WERE MORE ENTRIES - ALL COMMENTED OUT
...
!.... CALCULATE chargesq(j) HERE INSTEAD OF IN main TO BE UP TO DATE
      chargesq(j) = xne(j) + xne(j)
      excess = chargesq(j) - p_gas(j) / tk(j)
!.... ALLOWANCE FOR DOUBLY IONIZED HELIUM
      if(excess .gt. 0.0d0) chargesq(j) = chargesq(j) + excess + excess
      if(chargesq(j) .eq. 0.0d0) chargesq(j) = 1.0d0

!!!!! FOR TESTING RESET CONSTANTS TO BOB'S VALUES !!!!!!!!!!!!!!!!!!!!!!
      debye = sqrt(tk(j) / (pi4 * e_esu**2 * chargesq(j)) )
!!!!  debye = sqrt(tk(j) / 12.5664 / 4.801e-10**2 / chargesq(j)) !BOB'S
!!!!  debye = sqrt(tk(j) / (2.8965d-18 * chargesq(j)))           !BOB'S
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do ion = 1, nion2
         ...
         ip(ion) = potion(lower_lim(iz) + ion - 1)
         if(ip(ion) .eq. 0.0d0) ip(ion) = potion(lower_lim(iz) + ion -2)

!.... waveno_ev TO CONVERT ENERGIES FROM CM^-1 TO EV

         ip(ion) = ip(ion) * waveno_ev
...
!!!!        if(n .eq. 1) then                 !.... H I
               b = 1.0d0
!.... BOB USES nlteon = -1, I USE .false.
               if(nlteon) b = b_hyd(j, 1)
               part(1) = 2.0d0 * b

               if(t(j) .ge. 9000.0d0) then

                  do i = 2, 6
                     b = 1.0d0
                     if(nlteon) b = b_hyd(j, i)
                     part(1) = part(1) + g_hyd(i) * b *
     &                                   exp(-e_hyd(i) * hckt(j))
                  end do

!.... GROUND STATE IONIZATION ENERGY OF HYDROGEN IN CM-1
!....    BOB'S VALUE 109677.576
!!!!              d1 = 109677.576d0 / 6.5d0 / 6.5d0 * hckt(j)
!....    NIST'S VALUE 13.5984 / 1.23984244E-4 = 109678.452369 CM-1 
                  d1 = 109678.452369d0 / 6.5d0 / 6.5d0 * hckt(j)
                  d2 = potlo(ion) * tvi
                  part(ion) = part(ion) + 
     &                        finish_part(ip(ion))
               end if

            else if(iz .eq. 2 .and. ion .eq. 1) then !.... He I
!!!!        else if(n .eq. 3) then
               b = 1.0d0
               if(nlteon) b = b_he1(j, 1)
               part(1) = b

               if(t(j) .ge. 15000.0d0) then

                  do i = 2, 29
                     b = 1.0d0
                     if(nlteon) b = b_he1(j, i)
                     part(1) = part(1) + g_he1(i) * b *
     &                                   exp(-e_he1(i) * hckt(j))
                  end do
...
!....    NIST'S VALUE 13.5984 / 1.23984244E-4 = 109678.452369 CM-1 
                  d1 = 109678.452369d0 / 5.5d0 / 5.5d0 * hckt(j)
                  d2 = potlo(ion) * tvi
                  part(ion) = part(ion) + finish_part(ip(ion))
               end if

            else if(iz .eq. 2 .and. ion .eq. 2) then !.... He II
!!!!        else if(n .eq. 4) then
               b = 1.0d0
               if(nlteon) b = b_he2(j, 1)
               part(2) = 2.0d0 * b

               if(t(j) .ge. 30000.0d0) then

                  do i = 2, 6
                     b = 1.0d0
                     if(nlteon) b = b_he2(j, i)
                     part(2) = part(2) + g_he2(i) * b *
     &                                   exp(-e_he2(i) * hckt(j))
                  end do

                  d1 = 4.0d0 * 109722.267d0 / 6.5d0 / 6.5d0 * hckt(j)
                  d2 = potlo(ion) * tvi
                  part(ion) = part(ion) + finish_part(ip(ion))
               end if

            else if(iz .eq. 5 .and. ion .eq. 1) then !.... B I
!!!!        else if(n .eq. 14) then
               b = 1.0d0
               if(nlteon) b = b_b1(j, 1)
               part(1) = b * (2.0d0 + 4.0d0 * exp(-15.25d0 * hckt(j)))

               do i = 2, 7
                  b = 1.0d0
                  if(nlteon) b = b_b1(j, i)
                  part(1) = part(1) + g_b1(i) * b *
     &                                exp(-e_b1(i) * hckt(j))
               end do

               part(1) = part(1) +
     &                    6.0d0 * exp(-57786.80d0 * hckt(j)) +
     &                   10.0d0 * exp(-59989.00d0 * hckt(j)) +
     &                   14.0d0 * exp(-60031.03d0 * hckt(j)) +
     &                    2.0d0 * exp(-63561.00d0 * hckt(j))
               g = 2.0d0
               d1 = 109734.83d0 / 4.5d0 / 4.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 6 .and. ion .eq. 1) then !.... C I
!!!!        else if(n .eq. 354) then
               b = 1.0d0
               if(nlteon) b = b_c1(j, 1)
               part(1) = b * (1.0d0 +
     &                        3.0d0 * exp(-16.42d0 * hckt(j)) +
     &                        5.0d0 * exp(-43.42d0 * hckt(j)))

               do i = 2, 14
                  b = 1.0d0
                  if(nlteon) b = b_c1(j, i)
                  part(1) = part(1) + g_c1(i) * b *
     &                                exp(-e_c1(i) * hckt(j))
               end do

               part(1) = part(1) +
     &                   108.0d0 * exp(-80000.0d0 * hckt(j)) +
     &                   189.0d0 * exp(-84000.0d0 * hckt(j)) +
     &                   247.0d0 * exp(-87000.0d0 * hckt(j)) +
     &                   231.0d0 * exp(-88000.0d0 * hckt(j)) +
     &                   190.0d0 * exp(-89000.0d0 * hckt(j)) +
     &                   300.0d0 * exp(-90000.0d0 * hckt(j))

            else if(iz .eq. 6 .and. ion .eq. 2) then !.... C II
!!!!        else if(n .eq. 355) then
               b = 1.0d0
               if(nlteon) b = b_c2(j, 1)
               part(2) = b * (2.0d0 + 4.0d0 * exp(-63.42d0 * hckt(j)))

               do i = 2, 6
                  b = 1.0d0
                  if(nlteon) b = b_c2(j, i)
                  part(2) = part(2) + g_c2(i) * b *
     &                                exp(-e_c2(i)*hckt(j))
               end do

               part(2) = part(2) +
     &                    6.0d0 * exp(-131731.80d0 * hckt(j)) +
     &                    4.0d0 * exp(-142027.10d0 * hckt(j)) +
     &                   10.0d0 * exp(-145550.13d0 * hckt(j)) +
     &                   10.0d0 * exp(-150463.62d0 * hckt(j)) +
     &                    2.0d0 * exp(-157234.07d0 * hckt(j)) +
     &                    6.0d0 * exp(-162500.00d0 * hckt(j)) +
     &                   42.0d0 * exp(-168000.00d0 * hckt(j)) +
     &                   56.0d0 * exp(-178000.00d0 * hckt(j)) +
     &                  102.0d0 * exp(-183000.00d0 * hckt(j)) +
     &                  400.0d0 * exp(-188000.00d0 * hckt(j))

            else if(iz .eq. 8 .and. ion .eq. 1) then !.... O I
!!!!        else if(n .eq. 367) then
               b = 1.0d0
               if(nlteon) b = b_o1(j, 1)
               part(1) = b * (5.0d0 +
     &                        3.0d0 * exp(-158.265d0 * hckt(j)) +
     &                                exp(-226.977d0 * hckt(j)))

               do i = 2, 13
                  b = 1.0d0
                  if(nlteon) b = b_o1(j, i)
                  part(1) = part(1) + g_o1(i) * b *
     &                                exp(-e_o1(i) * hckt(j))
               end do

               part(1) = part(1) +
     &                    15.0d0 * exp(-101140.0d0 * hckt(j)) +
     &                   131.0d0 * exp(-103000.0d0 * hckt(j)) +
     &                   128.0d0 * exp(-105000.0d0 * hckt(j)) +
     &                   600.0d0 * exp(-107000.0d0 * hckt(j))

            else if(iz .eq. 11 .and. ion .eq. 1) then !.... Na I
!!!!        else if(n .eq. 45) then
               b = 1.0d0
               if(nlteon) b = b_na1(j, 1)
               part(1) = b * 2.0d0

               do i = 2, 8
                  b = 1.0d0
                  if(nlteon) b = b_na1(j, i)
                  part(1) = part(1) + g_na1(i) *
     &                                b * exp(-e_na1(i) * hckt(j))
               end do

               part(1) = part(1) +
     &                   10.0d0 * exp(-34548.745d0 * hckt(j)) +
     &                   14.0d0 * exp(-34586.960d0 * hckt(j))
               g = 2.0d0
               d1 = 109734.83d0 / 4.5d0 / 4.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 12 .and. ion .eq. 1) then !.... Mg I
!!!!        else if(n .eq. 51) then
               b = 1.0d0
               if(nlteon) b = b_mg1(j, 1)
               part(1) = b

               do i = 2, 11
                  b = 1.0d0
                  if(nlteon) b = b_mg1(j, i)
                  part(1) = part(1) + g_mg1(i) *
     &                                b * exp(-e_mg1(i) * hckt(j))
               end do

               part(1) = part(1) +
     &                    5.0d0 * exp(-53134.0d0 * hckt(j)) +
     &                   15.0d0 * exp(-54192.0d0 * hckt(j)) +
     &                   28.0d0 * exp(-54676.0d0 * hckt(j)) +
     &                    9.0d0 * exp(-57853.0d0 * hckt(j))
               g = 4.0d0
               d1 = 109734.83d0 / 4.5d0 / 4.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + 
     &                     finish_part(ip(ion))
            ...
            else if(iz .eq. 12 .and. ion .eq. 2) then !.... Mg II
!!!!        else if(n .eq. 52) then
               b = 1.0d0
               if(nlteon) b = b_mg2(j, 1)
               part(2) = b * 2.0d0

               do i = 2, 6
                  b = 1.0d0
                  if(nlteon) b = b_mg2(j, i)
                  part(2) = part(2) + g_mg2(i) * b *
     &                                exp(-e_mg2(i)*hckt(j))
               end do

               part(2) = part(2) +
     &                   10.0d0 * exp( -93310.80d0 * hckt(j)) +
     &                   14.0d0 * exp( -93799.70d0 * hckt(j)) +
     &                    6.0d0 * exp( -97464.32d0 * hckt(j)) +
     &                   10.0d0 * exp(-103419.82d0 * hckt(j)) +
     &                   14.0d0 * exp(-103689.89d0 * hckt(j)) +
     &                   18.0d0 * exp(-103705.66d0 * hckt(j))
               g = 2.0d0
               d1 = 4.0d0 * 109734.83d0 / 5.5d0 / 5.5d0 *hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 13 .and. ion .eq. 1) then !.... Al I
!!!!        else if(n .eq. 57) then
               b = 1.0d0
               if(nlteon) b = b_al1(j, 1)
               part(1) = b * (2.0d0 + 4.0d0 * exp(-112.061d0 * hckt(j)))

               do i = 2, 9
                  b = 1.0d0
                  if(nlteon) b = b_al1(j, i)
                  part(1) = part(1) + g_al1(i) *
     &                                b * exp(-e_al1(i) * hckt(j))
               end do

               part(1) = part(1) + 10.0d0 * exp(-42235.0d0 * hckt(j)) +
     &                             14.0d0 * exp(-43831.0d0 * hckt(j))
               g = 2.0d0
               d1 = 109735.08d0 / 5.5d0 / 5.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 14 .and. ion .eq. 1) then !.... Si I
!!!!        else if(n .eq. 63) then
               b = 1.0d0
               if(nlteon) b = b_si1(j, 1)
               part(1) = b * (1.0d0 +
     &                        3.0d0 * exp( -77.115d0 * hckt(j)) +
     &                        5.0d0 * exp(-223.157d0 * hckt(j)))

               do i = 2, 11
                  b = 1.0d0
                  if(nlteon) b = b_si1(j, i)
                  part(1) = part(1) + g_si1(i) *
     &                                b * exp(-e_si1(i) * hckt(j))
               end do

               part(1) = part(1) +  76.0d0 * exp(-53000.0d0 * hckt(j)) +
     &                              71.0d0 * exp(-57000.0d0 * hckt(j)) +
     &                             191.0d0 * exp(-60000.0d0 * hckt(j)) +
     &                             240.0d0 * exp(-62000.0d0 * hckt(j)) +
     &                             251.0d0 * exp(-63000.0d0 * hckt(j)) +
     &                             300.0d0 * exp(-65000.0d0 * hckt(j))

            else if(iz .eq. 14 .and. ion .eq. 2) then !.... Si II
!!!!        else if(n .eq. 64) then
               b = 1.0d0
               if(nlteon) b = b_si2(j, 1)
               part(2) = b * (2.0d0 + 4.0d0 * exp(-287.32d0 * hckt(j)))

               do i = 2, 6
                  b = 1.0d0
                  if(nlteon) b = b_si2(j, i)
                  part(2) = part(2) + g_si2(i) * b
     &                                * exp(-e_si2(i) * hckt(j))
               end do

               part(2) = part(2) +
     &                    6.0d0 * exp( -81231.59d0 * hckt(j)) +
     &                    6.0d0 * exp( -83937.08d0 * hckt(j)) +
     &                   10.0d0 * exp(-101024.09d0 * hckt(j)) +
     &                   14.0d0 * exp(-103556.35d0 * hckt(j)) +
     &                   10.0d0 * exp(-108800.00d0 * hckt(j)) +
     &                   42.0d0 * exp(-115000.00d0 * hckt(j)) +
     &                    6.0d0 * exp(-121000.00d0 * hckt(j)) +
     &                   38.0d0 * exp(-125000.00d0 * hckt(j)) +
     &                   34.0d0 * exp(-132000.00d0 * hckt(j))
               g = 2.0d0
               d1 = 4.0d0 * 109734.83d0 / 4.5d0 / 4.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 19 .and. ion .eq. 1) then !.... K I
!!!!        else if(n .eq. 91) then
               b = 1.0d0
               if(nlteon) b = b_k1(j, 1)
               part(1) = b * 2.0d0

               do i = 2, 8
                  b = 1.0d0
                  if(nlteon) b = b_k1(j, i)
                  part(1) = part(1) + g_k1(i) *
     &                                b * exp(-e_k1(i) * hckt(j))
               end do

               part(1) = part(1) +
     &                   10.0d0 * exp(-27397.077d0 * hckt(j)) +
     &                   14.0d0 * exp(-28127.850d0 * hckt(j))
               g = 2.0d0
               d1 = 109734.83d0 / 5.5d0 / 5.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 20 .and. ion .eq. 1) then !.... Ca I
!!!!        else if(n .eq. 96) then
               b = 1.0d0
               if(nlteon) b = b_ca1(j, 1)
               part(1) = b

               do i = 2, 8
                  b = 1.0d0
                  if(nlteon) b = b_ca1(j, i)
                  part(1) = part(1) + g_ca1(i) *
     &                                b * exp(-e_ca1(i) * hckt(j))
               end do

               part(1) = part(1) + 28.0d0 * exp(-37000.0d0 * hckt(j)) +
     &                             67.0d0 * exp(-40000.0d0 * hckt(j)) +
     &                             21.0d0 * exp(-43000.0d0 * hckt(j)) +
     &                             34.0d0 * exp(-48000.0d0 * hckt(j))
               g = 4.0d0
               d1 = 109734.82d0 / 4.5d0 / 4.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))

            else if(iz .eq. 20 .and. ion .eq. 2) then !.... Ca II
!!!!        else if(n .eq. 97) then
               b = 1.0d0
               if(nlteon) b = b_ca2(j, 1)
               part(2) = b * 2.0d0

               do i = 2, 5
                  b = 1.0d0
                  if(nlteon) b = b_ca2(j, i)
                  part(2) = part(2) + g_ca2(i) *
     &                                b * exp(-e_ca2(i) * hckt(j))
               end do

               part(2) = part(2) + 12.0d0 * exp(-68000.0d0 * hckt(j))
               g = 2.0d0
               d1 = 109734.83d0 / 4.5d0 / 4.5d0 * hckt(j)
               d2 = potlo(ion) * tvi
               part(ion) = part(ion) + finish_part(ip(ion))
               ...
               if(mod(it, 2) .eq. 0) then
                  p1 = real(k3, re_type) * scale(kscale)
                  k1 = nnn(i + 1, n) / 100000
                  kscale = mod(nnn(i + 1, n), 10)
                  p2 = real(k1, re_type) * scale(kscale)

               else
                  p1 = real(k1, re_type) * scale(kscale)
                  p2 = real(k3, re_type) * scale(kscale)
...
!.... NEW CODE FROM atlas7 08 SEP 2000 FOR THIS t(j)

!!!!!          if(t(j) .lt. t2000 * 1.5d0) then
               if(t(j) .lt. t2000 * 2.0d0) then  !.... CHANGED 2005

!.... CHANGED TO USE ion-1 IN THE CALL TO pfground INSTEAD OF ion 
!.... TO ALLOW CORRESPONDENCE WITH STANDARD SPECTROSCOPIC NOTATION

!!!!              pfg = pfground(iz, (ion-1), t(j))
!!!!              if(pfg .gt. 1.0d0) part(ion) = pfg
!.... PATCH 14JUN2004 AFTER JOHN LAIRD IN CASE pfground .gt. pfsaha
...
!.... THE USE OF MIN(75.0D0) CAN CAUSE A HUGE DIFFERENCE IN HIGH IONS
!!!!  USE FOR TESTING
!!!! &                   exp(-min((ip(1:nion2-1)-potlo(1:nion2-1))*tvi,
!!!! &                       75.0d0))
     ...
     &                        f(ion) * real(ion - 1, re_type)
      ...
      contains ! INTERNAL FUNCTION -------------------------------------
         ...
         function finish_part(ionp) result(f_part)
         real(re_type)             :: f_part
         real(re_type), intent(in) :: ionp

          f_part = g * exp(-ionp * tvi) * 
     &            (sqrt(hydip * z * z * tvi / d2)**3 * 
     &             (1.0d0 / 3.0d0 + d2 * (1.0d0 - 
     &                              d2 * (0.5d0 +
     &                              d2 * (1.0d0 / 18.0d0 +
     &                              d2 / 120.0d0))) )  -
     &             sqrt(hydip * z * z * tvi / d1 )**3 *
     &             (1.0d0 / 3.0d0 + d1 * (1.0d0 - 
     &                              d1 * (0.5d0 +
     &                              d1 * (1.0d0 / 18.0d0 +
     &                              d1 / 120.0d0))) ))
         end function finish_part
