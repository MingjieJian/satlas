...
!.... REFORMAT FLUX OUTPUT FROM SPECSYN
...
!------------------------- reform_f EXECUTION --------------------------
      ...
      open(unit = 6, file = 'reform_f.print', status = 'new',
...
!.... FILE 32 REC 1 = SURF_MU(1:N_MU) - NOT USED IN THIS SECTION
!.... FILE 32 REC 2 = WL_EDGE(1:N_EDGE) - NOT USED
!.... THEREFORE, START AT REC3 = hnu, surf(1)

         do i_wl = 1, len_spec
            w = wlbeg * spec_ratio**(i_wl - 1)
            read(32, rec = i_wl + 2) hnu, surf(1)
            write(6, '(i8, f12.4, es12.3, es12.3, f8.3)')
     &         i_wl, w, pi4 * hnu, pi4 * surf(1), hnu/surf(1)
         end do

      else if(if_int .or. n_mu .gt. 1) then
         write(6, '(a /
     &              a, a7, a8, a9, a7, a9, a10 /
     &              a, 6x, a4, a10,    a19, a8 /
     &              a)')
     &      "#",
     &      "#", "POINT", "LAMBDA", "ANGLE", "MU", "r/",      "I/",
     &      "#",           "(nm)",  "(deg)",       "R_star",  "I_c",
     &      "#"

         read(32, rec = 1) surf_mu(1:n_mu)
!!!!     read(32, rec = 2) wl_edge(1:n_edge) ! SKIP - NOT USED
         ...
         do i_wl = 1, len_spec
            ...
            write(6, '(i8, f10.3, f8.2, f8.3, f10.3, f10.3 /
     &                (18x,       f8.2, f8.2, f10.3, f10.3) )')
     &         i_wl, w, (surf_angle(i_mu), surf_mu(i_mu), surf_r(i_mu),
     &                   surf_int(i_mu)/surf(i_mu), i_mu = 1, n_mu)
         end do
      ...
      end program reform_f

!***************** E N D  P R O G R A M  R E F O R M_F *****************
